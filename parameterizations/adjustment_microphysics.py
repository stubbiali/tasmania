...
class AdjustmentMicrophysicsKesslerWRF(AdjustmentMicrophysics):
	...
	def __call__(self, dt, state):
		...
		ps = self._p0 * np.exp(self._alpha * (T - self._Tr) / (T - self._bw))
		...
		self._stencils_set_inputs(dt, rho, p, ps, exn, T, qv, qc, qr)
	...
	def _stencils_set_inputs(self, dt, rho, p, ps, exn, T, qv, qc, qr):
		"""
