...
class LaxWendroffSWES:
	...
	For ease of tractability, the equations are considered
	in a latitudinal band which does not include the poles,
	so to bypass the pole problem. Neumann conditions
	are applied on the resulting artificial boundaries.
	"""
	def __init__(self, planet, t_final, m, n, ic, cfl, diff, backend,
		...
		self.theta_1d = np.linspace(- 85. / 180. * math.pi, 85. / 180. * math.pi, self.n,
									dtype=dtype)
		...
		# print(self.dy1.shape)

		# np.save("dy1_setup.npy", self.dy1)
	...
	def solve(self, verbose, save):
		...
		if self.only_advection:
			# print("in_h", self.h.shape, "dy1", self.dy1.shape)
			# print(self.m+1, self.n-2)
