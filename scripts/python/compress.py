...
# ==================================================
filename_from = (
    "../../data/isentropic_dry_rk3cosmo_fifth_order_upwind_"
    "nx161_ny161_nz60_dt12_nt3600_gaussian_L50000_H1000_u15_f_cc.nc"
)
filename_to = (
    "../../data/compressed/isentropic_dry_rk3cosmo_fifth_order_upwind_"
    "nx161_ny161_nz60_dt12_nt3600_gaussian_L50000_H1000_u15_f_cc.nc"
)

field_names = ["height_on_interface_levels", "x_momentum_isentropic"]
        ...
        for name in field_names:
            if state_from[name].shape[2] == grid.nz:
                state_to[name] = taz.make_data_array_3d(
                    state_from[name].values[x, y, z],
                    r_grid,
                    units=state_from[name].attrs["units"],
                )
            else:
                state_to[name] = taz.make_data_array_3d(
                    state_from[name].values[x, y, z_hl],
                    r_grid,
                    units=state_from[name].attrs["units"],
                )
