...
if __name__ == '__main__':
	...
	for i in range(len(err)):
		pu.make_lineplot(
			...
			marker=markers[i], markersize=markersizes[i], markeredgecolor=markeredgecolors[i],
			markerfacecolor=markerfacecolors[i], legend_label=labels[i],
		)

	pu.make_lineplot(dx, 1e-1*dx**2, ax1, linecolor='black', linestyle='--', linewidth=1.5)
	pu.make_lineplot(dx, 1e-9*(dx/dx[-1])**4, ax1, linecolor='black', linestyle='--', linewidth=1.5)

	plt.text(
		dx[-1], 1e-1*dx[-1]**2, ' $\\mathcal{O}(\\Delta t)$',
		horizontalalignment='left', verticalalignment='center'
	)
	plt.text(
		dx[-1], 1e-9, ' $\\mathcal{O}(\\Delta t^2)$',
		horizontalalignment='left', verticalalignment='center'
	)
