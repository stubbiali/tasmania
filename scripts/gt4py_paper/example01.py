...
@gtscript.function
def laplacian(phi):
    out = -4 * phi[0, 0, 0] + phi[-1, 0, 0] + phi[1, 0, 0] + phi[0, -1, 0] + phi[0, 1, 0]
    return out


@gtscript.function
def gradx(phi):
    out = phi[1, 0, 0] - phi[0, 0, 0]
    return out


@gtscript.function
def grady(phi):
    out = phi[0, 1, 0] - phi[0, 0, 0]
    return out
...
def diffusion_defs(
    ...
    from __externals__ import gradx, grady, laplacian
    ...
    with computation(PARALLEL), interval(...):
        ...
        # update the field
        out_phi = in_phi + alpha * (gradx(flux_x[-1, 0, 0]) + grady(flux_y[0, -1, 0]))
...
if __name__ == "__main__":
    ...
    diffusion = gtscript.stencil(
        ...
        diffusion_defs,
        dtypes={"dtype": dtype},
        externals={"gradx": gradx, "grady": grady, "laplacian": laplacian},
        verbose=verbose,
    )
