...
* :data:`namelist.smooth_type`: Type of smoothing technique to implement. Available options are:
		...
		- 'first-order', for first-order smoothing;
		- 'second-order', for second-order smoothing.
...
damp_max               = .0002
ismooth                = True
smooth_type            = 'first-order'
smooth_damp_depth      = 0
smooth_coeff           = .03
smooth_coeff_max       = .03
smooth_coeff_moist     = .03
smooth_coeff_moist_max = .03
...
save_iterations		  = []
save_dest		      = os.path.join(os.environ['TASMANIA_ROOT'], 'data/verification_1_leapfrog.pickle')
tol      		      = 1.e-8		
datatype 		      = np.float32
