...
class WrappingStencil:
	...
	def __call__(
		self, dt, s, u, v, su, sv, ddmtg, sqv=None, sqc=None, sqr=None,
		...
		outputs = {
			...
			'flux_ddmtg_x': self.flux_ddmtg_x, 'flux_ddmtg_y': self.flux_ddmtg_y,
		}
		if sqv is not None:
	...
	def stencil_defs(
		...
		fs = self.core(
			i, j, dt, s, u, v, su, sv, ddmtg, sqv, sqc, sqr,
		...
		if len(fs) == 8:
			...
			return fs[0], fs[1], fs[2], fs[3], fs[4], fs[5], fs[6], fs[7], \
				fs[8], fs[9], fs[10], fs[11], fs[12], fs[13]


flux_properties = {
	'upwind':
		{'type': Upwind, 'get_fluxes': get_upwind_fluxes},
	'centered':
		{'type': Centered, 'get_fluxes': get_centered_fluxes},
	'third_order_upwind':
		{'type': ThirdOrderUpwind, 'get_fluxes': get_third_order_upwind_fluxes},
	'fifth_order_upwind':
		{'type': FifthOrderUpwind, 'get_fluxes': get_fifth_order_upwind_fluxes},
}
...
def validation(flux_scheme, domain, field, timestep, backend):
	...
	out = fluxer_moist(
		i, j, dt, s_eq, u_eq, v_eq, su_eq, sv_eq, ddmtg_eq,
	...
	compare_arrays(ws.flux_ddmtg_x[nb-1:-nb, nb-1:-nb], flux_ddmtg_x[nb-1:-nb, nb-1:-nb])
	compare_arrays(ws.flux_ddmtg_y[nb-1:-nb, nb-1:-nb], flux_ddmtg_y[nb-1:-nb, nb-1:-nb])
