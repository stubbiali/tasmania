...
def test_numerics_forward_euler(
	...
	assert np.allclose(f2, 2*s0, equal_nan=True)
	...
	assert np.allclose(
		state_prv['x_momentum_isentropic'].to_units('kg m^-1 K^-1 s^-1').values, su2, equal_nan=True
	)
	...
	assert np.allclose(
		state_prv['x_velocity_at_u_locations'].to_units('m s^-1').values, u2, equal_nan=True
	)
...
def test_numerics_rk2(
	...
	assert 'fake_variable' in state
	s0 = state_dc['air_isentropic_density'].to_units('kg m^-2 K^-1').values
	f2 = state['fake_variable'].to_units('kg m^-2 K^-1').values
	assert np.allclose(f2, 2*s0, equal_nan=True)
	...
	s3 = s2 + timestep.total_seconds() * 0.01 * f2
	assert np.allclose(
		state_prv['air_isentropic_density'].to_units('kg m^-2 K^-1').values, s3, equal_nan=True
	)
	...
	sv1 = state_prv_dc['y_momentum_isentropic'].to_units('kg m^-1 K^-1 s^-1').values
	sv3b = 0.5 * (sv0 + sv1 + timestep.total_seconds() * 0.5 * s0 * (v0[:, :-1, :] + v0[:, 1:, :]))
...
def _test_numerics_substepping(
	...
	assert np.allclose(
		state_prv['x_velocity_at_u_locations'].values, u2, equal_nan=True
	)
