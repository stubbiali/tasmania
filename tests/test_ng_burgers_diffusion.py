...
try:
    ...
    from .utils import st_ng_burgers_state, st_domain, st_floats, st_one_of
...
def test_second_order(data):
    ...
    pstate = data.draw(
        st_ng_burgers_state(pgrid, backend=backend, dtype=dtype, halo=halo),
        label="pstate",
    )
    cstate = data.draw(
        st_ng_burgers_state(cgrid, backend=backend, dtype=dtype, halo=halo),
        label="cstate",
    )
    ...
    # physical grid
    #
    pbhd = NGBurgersHorizontalDiffusion(
        domain,
        "physical",
        "second_order",
    ...
    # numerical grid
    #
    cbhd = NGBurgersHorizontalDiffusion(
        domain,
        "numerical",
        "second_order",
        DataArray(smooth_coeff, attrs={"units": "m^2 s^-1"}),
