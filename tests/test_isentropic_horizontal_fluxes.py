...
class WrappingStencil:
	...
	def __call__(
		self, dt, dx, dy, s, u, v, mtg, su, sv, sqv=None, sqc=None, sqr=None,
		...
		if moist:
			stencil_args['sqv'] = gt.storage.from_array(sqv, descriptor, backend=self.backend)
			stencil_args['flux_sqv_x'] = gt.storage.zeros(descriptor, backend=self.backend)
			stencil_args['flux_sqv_y'] = gt.storage.zeros(descriptor, backend=self.backend)
			stencil_args['sqc'] = gt.storage.from_array(sqc, descriptor, backend=self.backend)
			stencil_args['flux_sqc_x'] = gt.storage.zeros(descriptor, backend=self.backend)
			stencil_args['flux_sqc_y'] = gt.storage.zeros(descriptor, backend=self.backend)
			stencil_args['sqr'] = gt.storage.from_array(sqr, descriptor, backend=self.backend)
			stencil_args['flux_sqr_x'] = gt.storage.zeros(descriptor, backend=self.backend)
			stencil_args['flux_sqr_y'] = gt.storage.zeros(descriptor, backend=self.backend)
		s_tnd_on = s_tnd is not None
		stencil_args['s_tnd'] = stencil_args['s'] if not s_tnd_on else \
			gt.storage.from_array(s_tnd, descriptor, backend=self.backend)
		su_tnd_on = su_tnd is not None
		stencil_args['su_tnd'] = stencil_args['su'] if not su_tnd_on else \
			gt.storage.from_array(su_tnd, descriptor, backend=self.backend)
		sv_tnd_on = sv_tnd is not None
		stencil_args['sv_tnd'] = stencil_args['sv'] if not sv_tnd_on else \
			gt.storage.from_array(sv_tnd, descriptor, backend=self.backend)
		qv_tnd_on = qv_tnd is not None
		if moist:
			stencil_args['qv_tnd'] = stencil_args['sqv'] if not qv_tnd_on else \
				gt.storage.from_array(qv_tnd, descriptor, backend=self.backend)
		qc_tnd_on = qc_tnd is not None
		if moist:
			stencil_args['qc_tnd'] = stencil_args['sqc'] if not qc_tnd_on else \
				gt.storage.from_array(qc_tnd, descriptor, backend=self.backend)
		qr_tnd_on = qr_tnd is not None
		if moist:
			stencil_args['qr_tnd'] = stencil_args['sqr'] if not qr_tnd_on else \
				gt.storage.from_array(qr_tnd, descriptor, backend=self.backend)
		...
		stencil(
			**stencil_args, dt=dt, dx=dx, dy=dy,
			origin={'_all_': (nb-1, nb-1, 0)}, domain=(mi-2*nb, mj-2*nb, mk)
		)
		...
		if moist:
			return_list_names += [
				'flux_sqv_x', 'flux_sqv_y', 'flux_sqc_x', 'flux_sqc_y',
	...
	@staticmethod
	def stencil_defs(
		s: gt.storage.f64_sd, u: gt.storage.f64_sd, v: gt.storage.f64_sd,
		mtg: gt.storage.f64_sd, su: gt.storage.f64_sd, sv: gt.storage.f64_sd,
		sqv: gt.storage.f64_sd, sqc: gt.storage.f64_sd, sqr: gt.storage.f64_sd,
		s_tnd: gt.storage.f64_sd, su_tnd: gt.storage.f64_sd, sv_tnd: gt.storage.f64_sd,
		qv_tnd: gt.storage.f64_sd, qc_tnd: gt.storage.f64_sd, qr_tnd: gt.storage.f64_sd,
		flux_s_x: gt.storage.f64_sd, flux_s_y: gt.storage.f64_sd,
		flux_su_x: gt.storage.f64_sd, flux_su_y: gt.storage.f64_sd,
		flux_sv_x: gt.storage.f64_sd, flux_sv_y: gt.storage.f64_sd,
		flux_sqv_x: gt.storage.f64_sd, flux_sqv_y: gt.storage.f64_sd,
		flux_sqc_x: gt.storage.f64_sd, flux_sqc_y: gt.storage.f64_sd,
		flux_sqr_x: gt.storage.f64_sd, flux_sqr_y: gt.storage.f64_sd,
		*, dt: float, dx: float, dy: float
	):
		if not moist:
			flux_s_x, flux_s_y, flux_su_x, flux_su_y, flux_sv_x, flux_sv_y = \
				core(
					dt=dt, dx=dx, dy=dy, s=s, u=u, v=v, mtg=mtg, su=su, sv=sv,
			...
			flux_s_x, flux_s_y, flux_su_x, flux_su_y, flux_sv_x, flux_sv_y, \
				...
				core(
					dt=dt, dx=dx, dy=dy, s=s, u=u, v=v, mtg=mtg, su=su, sv=sv,
					sqv=sqv, sqc=sqc, sqr=sqr, s_tnd=s_tnd, su_tnd=su_tnd, sv_tnd=sv_tnd,
...
def validation(flux_scheme, domain, field, timestep, backend, dtype, halo, rebuild):
	...
	# moist
	#
	fsx, fsy, fsux, fsuy, fsvx, fsvy, fsqvx, fsqvy, fsqcx, fsqcy, fsqrx, fsqry = \
		ws(timestep, dx, dy, s, u, v, mtg, su, sv, sqv=sqv, sqc=sqc, sqr=sqr)
...
def _test_upwind(data):
	...
	field = data.draw(
		st_arrays(
			dtype, (grid.nx+2, grid.ny+2, grid.nz+1),
			elements=st_floats(),
			fill=hyp_st.nothing(),
		)
	)
...
def _test_third_order_upwind(data):
	...
	# test bed
	# ========================================
	validation('third_order_upwind', domain, field, timestep, backend, dtype, halo, rebuild=True)
...
def test_fifth_order_upwind(data):
	...
	# test bed
	# ========================================
	validation('fifth_order_upwind', domain, field, timestep, backend, dtype, halo, rebuild=True)
