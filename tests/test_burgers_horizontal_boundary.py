...
from tasmania.python.burgers.dynamics.horizontal_boundary import \
...
def test_zhao_test_case_hb(data):
	...
	nb = data.draw(hyp_st.integers(min_value=1, max_value=int(min(grid.nx, grid.ny)/2)))
	time = data.draw(
		hyp_st.datetimes(
	...
	assert np.allclose(
		u[:nb, :, :],
		zsf(grid, time-state['time'], slice_x=slice(0, nb), field_name='x_velocity'),
		equal_nan=True,
	)
	assert np.allclose(
		u[-nb:, :, :],
		zsf(grid, time-state['time'], slice_x=slice(grid.nx-nb, grid.nx), field_name='x_velocity'),
		equal_nan=True,
	)
	assert np.allclose(
		u[:, :nb, :],
		zsf(grid, time-state['time'], slice_y=slice(0, nb), field_name='x_velocity'),
		equal_nan=True,
	)
	assert np.allclose(
		u[:, -nb:, :],
		zsf(grid, time-state['time'], slice_y=slice(grid.ny-nb, grid.ny), field_name='x_velocity'),
		equal_nan=True,
	)
	...
	assert np.allclose(
		v[:nb, :, :],
		zsf(grid, time-state['time'], slice_x=slice(0, nb), field_name='y_velocity'),
		equal_nan=True,
	)
	assert np.allclose(
		v[-nb:, :, :],
		zsf(grid, time-state['time'], slice_x=slice(grid.nx-nb, grid.nx), field_name='y_velocity'),
		equal_nan=True,
	)
	assert np.allclose(
		v[:, :nb, :],
		zsf(grid, time-state['time'], slice_y=slice(0, nb), field_name='y_velocity'),
		equal_nan=True,
	)
	assert np.allclose(
		v[:, -nb:, :],
		zsf(grid, time-state['time'], slice_y=slice(grid.ny-nb, grid.ny), field_name='y_velocity'),
		equal_nan=True,
	)
