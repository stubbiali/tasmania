...
try:
    ...
    from .utils import st_ng_burgers_state, st_domain, st_one_of, st_timedeltas
...
def test_forward_euler(data):
    ...
    compare_arrays(u1, new_state["x_velocity"].values)

    assert new_state["y_velocity"].attrs["units"] == "m s^-1"
    compare_arrays(v1, new_state["y_velocity"].values)
...
def test_rk2(data):
    ...
    assert new_state["x_velocity"].attrs["units"] == "m s^-1"
    compare_arrays(u2, new_state["x_velocity"].values)

    assert new_state["y_velocity"].attrs["units"] == "m s^-1"
    compare_arrays(v2, new_state["y_velocity"].values)
...
def test_rk3ws(data):
    ...
    assert new_state["x_velocity"].attrs["units"] == "m s^-1"
    compare_arrays(u3, new_state["x_velocity"].values)

    assert new_state["y_velocity"].attrs["units"] == "m s^-1"
    compare_arrays(v3, new_state["y_velocity"].values)
