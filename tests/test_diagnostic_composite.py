...
def isentropic_diagnostics_validation(grid, state, cp, p_ref, rd, g):
	...
	# density
	r_val = s * (theta[:, :, :-1] - theta[:, :, 1:]) / (h_val[:, :, :-1] - h_val[:, :, 1:])

	# temperature
	temp_val = 0.5 * (theta[:, :, :-1] * exn_val[:, :, :-1] + theta[:, :, 1:] * exn_val[:, :, 1:]) / cp
...
def test_serial(data):
	...
	dv = IsentropicDiagnostics(
		domain, grid_type, True, state['air_pressure_on_interface_levels'][0, 0, 0],
		backend=gt.mode.NUMPY, dtype=dtype
	)
	sa = SaturationAdjustmentKessler(
		domain, grid_type, air_pressure_on_interface_levels=True,
	...
	p_val, exn_val, mtg_val, h_val, r_val, temp_val = \
