...
def st_isentropic_state(draw, grid, *, time=None, moist=False, precipitation=False):
    ...
    return_dict["x_velocity_at_u_locations"] = draw(
        st_field(
            grid, "isentropic_state", "x_velocity_at_u_locations", (nx + 1, ny, nz)
        )
    )
    ...
    su_raw = (
        s.to_units("kg m^-2 K^-1").values
        * 0.5
        * (
            u.to_units("m s^-1").values[:-1, :, :]
            + u.to_units("m s^-1").values[1:, :, :]
        )
    )
    ...
    return_dict["y_velocity_at_v_locations"] = draw(
        st_field(
            grid, "isentropic_state", "y_velocity_at_v_locations", (nx, ny + 1, nz)
        )
    )
    ...
    mtg_s = (
        grid.z_on_interface_levels.to_units("K").values[-1] * exn[:, :, -1]
    ...
    for k in range(nz, 0, -1):
        h[:, :, k - 1] = h[:, :, k] - Rd * (
            theta[:, :, k - 1] * exn[:, :, k - 1] + theta[:, :, k] * exn[:, :, k]
    ...
    if moist:
        ...
        if precipitation:
            ...
            return_dict["precipitation"] = draw(
                st_field(grid, "isentropic_state", "precipitation", (nx, ny, 1))
            )
...
def st_isentropic_boussinesq_state_f(
	...
	"""
    return_dict = draw(
        st_isentropic_state_f(grid, time=time, moist=moist, precipitation=precipitation)
    )
