...
def test_relaxed(data):
    ...
    if nx == 1:
        ...
        assert all(
            [
                eq(cx.values[i], x.values[0] - nb + i)
    ...
    if ny == 1:
        ...
        assert all(
            [
                eq(cy.values[i], y.values[0] - nb + i)
                for i in range(2 * nb + 1)
                if i != nb
            ]
        )
    ...
    if ny == 1:
        ...
        assert np.allclose(
            hb.get_numerical_field(field_stgx),
            np.repeat(field_stgx, 2 * nb + 1, axis=1),
        )
        assert np.allclose(
        ...
        assert np.allclose(
            hb.get_numerical_field(field_stgy),
            np.repeat(field_stgy, 2 * nb + 1, axis=0),
        )
    else:
...
def test_periodic(data):
    ...
    if nx == 1:
        ...
        assert all(
            [
                eq(
                    cx.coords[cx.dims[0]].values[i + 1]
                    - cx.coords[cx.dims[0]].values[i],
                    dx,
                )
    ...
    if ny == 1:
        ...
        assert all(
            [
                eq(
                    cy.coords[cy.dims[0]].values[i + 1]
                    - cy.coords[cy.dims[0]].values[i],
                    dy,
                )
