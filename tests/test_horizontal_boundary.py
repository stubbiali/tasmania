...
from tasmania.python.grids._horizontal_boundary import \
	Relaxed, Relaxed1DX, Relaxed1DY, \
	Periodic, Periodic1DX, Periodic1DY, \
	Dirichlet, Dirichlet1DX, Dirichlet1DY, \
	Identity, Identity1DX, Identity1DY
...
def test_relaxed(data):
	...
	cstate = data.draw(
		utils.st_isentropic_state(cgrid, moist=True, precipitation=True), label="ref_state"
	)
	...
	if nx == 1:
		...
		assert all([eq(cx.values[i], x.values[0]-nb+i) for i in range(2*nb+1) if i != nb])
		assert cx.values[nb] == x.values[0]
		assert len(cx.coords) == 1
		assert all([
			eq(cx.coords[cx.dims[0]].values[i], x.values[0]-nb+i)
			for i in range(2*nb+1) if i != nb
		])
		...
		assert all([eq(cx.values[i], xu.values[1]-nb-1+i) for i in range(nb+2, 2*nb+2)])
		assert len(cx.coords) == 1
		assert all([
	...
	utils.compare_dataarrays(
		x, hb.get_physical_xaxis(hb.get_numerical_xaxis(x))
	)
	utils.compare_dataarrays(
		xu, hb.get_physical_xaxis(hb.get_numerical_xaxis(xu))
	)
	...
	if ny == 1:
		...
		assert all([eq(cy.values[i], y.values[0]-nb+i) for i in range(2*nb+1) if i != nb])
		...
		assert all([eq(cy.values[i], yv.values[1]-nb-1+i) for i in range(nb+2, 2*nb+2)])
		assert len(cy.coords) == 1
		assert all([
	...
	utils.compare_dataarrays(
		y, hb.get_physical_yaxis(hb.get_numerical_yaxis(y))
	)
	utils.compare_dataarrays(
		yv, hb.get_physical_yaxis(hb.get_numerical_yaxis(yv))
	)
	...
	assert np.allclose(
		hb.get_physical_field(hb.get_numerical_field(field)), field
	)
	...
	hb.set_outermost_layers_x(
		...
		field_name='x_velocity_at_u_locations', field_units=units,
	)
	assert np.allclose(
	...
	hb.set_outermost_layers_y(
		...
		field_name='y_velocity_at_v_locations', field_units=units,
	)
	assert np.allclose(
...
def test_dirichlet_burgers(data):
	...
	state = data.draw(
		utils.st_burgers_state(grid), label="state"
	)

	cgrid = NumericalGrid(grid, hb)
	cstate = data.draw(
		utils.st_burgers_state(cgrid), label="ref_state"
	)
