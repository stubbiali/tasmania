...
def kessler_validation(
    ...
    exn = (
        exn if exn.shape[2] == rho.shape[2] else 0.5 * (exn[:, :, :-1] + exn[:, :, 1:])
    )
...
def _test_kessler_microphysics(data):
    ...
    compare_dataarrays(
        make_dataarray_3d(tnd_qc, grid, "g g^-1 s^-1"),
        tendencies[mfcw],
        compare_coordinate_values=False,
    )
    compare_dataarrays(
        make_dataarray_3d(tnd_qr, grid, "g g^-1 s^-1"),
        tendencies[mfpw],
        compare_coordinate_values=False,
    )
    if mfwv in tendency_names:
        compare_dataarrays(
            make_dataarray_3d(tnd_qv, grid, "g g^-1 s^-1"),
            tendencies[mfwv],
            compare_coordinate_values=False,
        )
...
def _test_kessler_saturation_adjustment(data):
    ...
    compare_dataarrays(
        make_dataarray_3d(out_qv, grid, "g g^-1"),
        diagnostics[mfwv],
        compare_coordinate_values=False,
    )
    compare_dataarrays(
        make_dataarray_3d(out_qc, grid, "g g^-1"),
        diagnostics[mfcw],
        compare_coordinate_values=False,
    )
...
def _test_kessler_fall_velocity(data):
    ...
    compare_dataarrays(
        make_dataarray_3d(vt, grid, "m s^-1"),
...
def test_kessler_sedimentation(data):
    ...
    rfv = KesslerFallVelocity(
        domain, grid_type, backend=backend, dtype=dtype, halo=halo
    )
    diagnostics = rfv(state)
