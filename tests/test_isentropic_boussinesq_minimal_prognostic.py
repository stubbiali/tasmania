...
def test_leapfrog(data):
    ...
    nb = (
        1
    )  # TODO: nb = data.draw(hyp_st.integers(min_value=1, max_value=max(1, conf_nb))
    domain = data.draw(
...
def test_rk2(data):
    ...
    nb = (
        2
    )  # TODO: nb = data.draw(hyp_st.integers(min_value=2, max_value=max(2, conf_nb))
    domain = data.draw(
        st_domain(xaxis_length=(7, 30), yaxis_length=(7, 30), nb=nb), label="domain"
    )
...
def test_rk3(data):
    ...
    assert "time" in raw_state_3.keys()
    compare_datetimes(
        raw_state_3["time"], raw_state_2["time"] + (1 - a2 - a1) * timestep
    )
