...
def first_order_validation(phi_rnd, ni, nj, nk, smooth_depth, nb, backend, halo):
...
def test_first_order(data):
    ...
    first_order_validation(
        pphi_rnd, nx + dnx, ny + dny, nz + dnz, depth, nb, backend, halo
    )

    nx, ny, nz = cgrid.nx, cgrid.ny, cgrid.nz
    first_order_validation(
        cphi_rnd, nx + dnx, ny + dny, nz + dnz, depth, nb, backend, halo
    )
...
def second_order_validation(phi_rnd, ni, nj, nk, smooth_depth, nb, backend, halo):
    phi = phi_rnd[:ni, :nj, :nk]
    phi_new = np.zeros_like(phi)
...
def third_order_validation(phi_rnd, ni, nj, nk, smooth_depth, nb, backend, halo):
    phi = phi_rnd[:ni, :nj, :nk]
    phi_new = np.zeros_like(phi)
