...
#
from hypothesis import assume, given, HealthCheck, settings, strategies as hyp_st
...
class WrappingStencil:
    ...
    def __call__(self, w, s, su, sv, ddmtg, sqv=None, sqc=None, sqr=None):
        ...
        stencil = gt.NGStencil(
            ...
            inputs=inputs,
            outputs=outputs,
            domain=gt.domain.Rectangle((0, 0, self.nb), (mi - 1, mj - 1, mk - self.nb)),
            mode=self.backend,
        )
...
def validation(flux_scheme, domain, field, backend):
    ...
    # dry
    #
    fluxer_dry = IsentropicBoussinesqMinimalVerticalFlux.factory(flux_scheme, grid, False)
