...
class WrappingStencil:
	...
	def __call__(
		self, dt, dx, dy, s, u, v, su, sv, sqv=None, sqc=None, sqr=None,
	...
	def stencil_defs(
		...
		su: gt.storage.f64_sd, sv: gt.storage.f64_sd,
		...
		*, dt: float, dx: float, dy: float
		...
		if not moist:
			flux_s_x, flux_s_y, flux_su_x, flux_su_y, flux_sv_x, flux_sv_y = \
				core(
					dt=dt, dx=dx, dy=dy, s=s, u=u, v=v, su=su, sv=sv,
...
def test_upwind(data):
	...
	halo = data.draw(st_one_of(conf_halo), label="backend")
