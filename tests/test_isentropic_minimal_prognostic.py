...
def test_leapfrog(data):
    ...
    nb = (
        1
    )  # TODO: nb = data.draw(hyp_st.integers(min_value=1, max_value=max(1, conf.nb))
    domain = data.draw(
...
def test_rk2(data):
    ...
    nb = (
        2
    )  # TODO: nb = data.draw(hyp_st.integers(min_value=2, max_value=max(2, conf.nb))
    domain = data.draw(
        st_domain(xaxis_length=(7, 30), yaxis_length=(7, 30), nb=nb), label="domain"
    )
...
def rk3_first_step(
    get_fluxes, mode, dx, dy, dt, a1, u, v, phi, phi_tnd, phi_k0, phi_out
):
...
def test_rk3ws_substepping(data):
    ...
    if moist:
        substep_output_properties.update(
            {mfwv: "g g^-1", mfcw: "g g^-1", mfpw: "g g^-1"}
        )
