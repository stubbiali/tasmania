...
def _test_porz_microphysics(data):
    ...
    compare_dataarrays(
        get_dataarray_3d(tnd_qv, grid, "kg kg^-1 s^-1"),
        tendencies[mfwv],
        compare_coordinate_values=False,
    )
    compare_dataarrays(
        get_dataarray_3d(tnd_qc, grid, "kg kg^-1 s^-1"),
        tendencies[mfcw],
        compare_coordinate_values=False,
    )
    compare_dataarrays(
        get_dataarray_3d(tnd_qr, grid, "kg kg^-1 s^-1"),
        tendencies[mfpw],
        compare_coordinate_values=False,
    )
    compare_dataarrays(
        get_dataarray_3d(tnd_nr, grid, "kg^-1 s^-1"),
        tendencies[ndpw],
        compare_coordinate_values=False,
    )
...
def test_porz_fall_velocity(data):
    ...
    compare_dataarrays(
        get_dataarray_3d(vq, grid, "m s^-1"),
        diagnostics["raindrop_fall_velocity"],
        compare_coordinate_values=False,
    )
    compare_dataarrays(
        get_dataarray_3d(vn, grid, "m s^-1"),
...
def test_porz_sedimentation(data):
    ...
    sed = PorzSedimentation(
        domain, grid_type, flux_type, maxcfl, backend=backend, dtype=dtype, rebuild=True
    )
