...
def _test_field(data):
	...
	state = data.draw(
		utils.st_isentropic_state_f(grid, moist=True, precipitation=True),
		label="state"
	)
	...
	data_val = state[field_name].to_units(field_units).values \
		if field_units is not None else state[field_name].values
...
def _test_horizontal_velocity(data):
	...
	data_val = hv[x, y, z] if field_units is None else \
		make_dataarray_3d(hv, grid, 'm s^-1')[x, y, z].to_units(field_units).values
...
def _test_height(data):
	...
	assert data.shape[1] == ymax - ymin
	assert data.shape[2] == zmax - zmin
	factor = DataArray(1, attrs={'units': 'm'}).to_units(field_units).values.item() \
		if field_units is not None else 1.0
...
def test_composite(data):
	...
	xmax1 = data.draw(hyp_st.integers(min_value=xmin1+1, max_value=grid.nx), label="xmax1")
	ymin1 = data.draw(hyp_st.integers(min_value=0, max_value=grid.ny-1), label="ymin1")
	ymax1 = data.draw(hyp_st.integers(min_value=ymin1+1, max_value=grid.ny), label="ymax1")
	zmin1 = data.draw(hyp_st.integers(min_value=0, max_value=grid.nz-1), label="zmin1")
	zmax1 = data.draw(hyp_st.integers(min_value=zmin1+1, max_value=grid.nz), label="zmax1")
	...
	xmax2 = data.draw(hyp_st.integers(min_value=xmin2+1, max_value=grid.nx), label="xmax2")
	ymin2 = data.draw(hyp_st.integers(min_value=0, max_value=grid.ny-1), label="ymin2")
	ymax2 = data.draw(hyp_st.integers(min_value=ymin2+1, max_value=grid.ny), label="ymax2")
	zmin2 = data.draw(hyp_st.integers(min_value=0, max_value=grid.nz-1), label="zmin2")
	zmax2 = data.draw(hyp_st.integers(min_value=zmin2+1, max_value=grid.nz), label="zmax2")
	...
	f1_units = field1_units if field1_units is not None else \
		state[field1_name].attrs['units']
	assert np.allclose(data[0], state[field1_name][x1, y1, z1].to_units(f1_units))

	# two input states
	drc = DataRetrieverComposite(
		(grid, grid), ((field1_name, ), (field2_name, )),
	...
	f2_units = field2_units if field2_units is not None else \
		state[field2_name].attrs['units']
