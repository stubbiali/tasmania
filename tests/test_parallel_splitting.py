...
from tasmania.core.physics_composite import ParallelSplitting
...
def test_compatibility(
	...
	ps1 = ParallelSplitting(tendency1, tendency2, grid=grid, mode='as_parallel')
	try:
		_ = ps1(state=state_dc, state_prv=state_dc, timestep=timestep)
		assert False
	...
	#
	state_dc = deepcopy(state)
	ps2 = ParallelSplitting(tendency2, tendency1, grid=grid, mode='serial')
	try:
		_ = ps2(state=state_dc, state_prv=state_dc, timestep=timestep)
		assert False
	except InvalidStateError:
		assert True
	...
	#
	state_dc = deepcopy(state)
	ps3 = ParallelSplitting(tendency1, tendency2, grid=grid, mode='serial')
	try:
		_ = ps3(state=state_dc, state_prv=state_dc, timestep=timestep)
		assert True
	except InvalidStateError:
		assert False
...
def test_numerics_forward_euler(
	...
	ps = ParallelSplitting(
		tendency1, tendency2, grid=grid, mode='serial', time_integration_scheme='forward_euler'
	)
	_ = ps(state=state_dc, state_prv=state_prv_dc, timestep=timestep)
...
def test_numerics_rk2(
	...
	ps = ParallelSplitting(
		tendency1, tendency2, grid=grid, mode='serial', time_integration_scheme='rk2'
	)
	_ = ps(state=state_dc, state_prv=state_prv_dc, timestep=timestep)
