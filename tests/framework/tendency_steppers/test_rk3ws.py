...
def test(data, backend, dtype, make_fake_tendency_component_1):
    ...
    assert units_are_same(
        rk3.output_properties["air_isentropic_density"]["units"],
        "kg m^-2 K^-1",
    )
    assert "x_momentum_isentropic" in rk3.output_properties
    assert units_are_same(
        rk3.output_properties["x_momentum_isentropic"]["units"],
        "kg m^-1 K^-1 s^-1",
    )
    assert "x_velocity" in rk3.output_properties
    assert units_are_same(
        rk3.output_properties["x_velocity"]["units"], "m s^-1"
    )
    ...
    vx1 = vx + 1.0 / 3.0 * dt.total_seconds() * vx_tnd
    ...
    state_1 = get_dataarray_dict(raw_state_1, ngrid, properties)
    ...
    vx_tnd = to_numpy(tendencies["x_velocity"].to_units("m s^-2").data)
    vx2 = vx + 0.5 * dt.total_seconds() * vx_tnd
    ...
    state_2 = get_dataarray_dict(raw_state_2, ngrid, properties)
    ...
    vx_tnd = to_numpy(tendencies["x_velocity"].to_units("m s^-2").data)
    vx3 = vx + dt.total_seconds() * vx_tnd
