...
try:
    ...
    from .utils import compare_dataarrays, st_domain, st_one_of, st_isentropic_state_f
...
def pressure_thickness_weighted_validation(dx, dy, eps, p, mtg):
    ...
    pgx_avg = (wgtx_w * pgx_w + wgtx_e * pgx_e + wgtx_s * pgx_s + wgtx_n * pgx_n) / (
    ...
    pgy_avg = (wgty_w * pgy_w + wgty_e * pgy_e + wgty_s * pgy_s + wgty_n * pgy_n) / (
...
def test_nonconservative(data):
    ...
    nb = (
        1
    )  # TODO: nb = 1 if domain.horizontal_boundary.nb < 1 else domain.horizontal_boundary.nb
    u_tnd[1:-1, 1:-1, :] = -(mtg[2:, 1:-1, :] - mtg[:-2, 1:-1, :]) / (2.0 * dx)
    u_tnd_val = get_dataarray_3d(u_tnd, grid, "m s^-2", name="x_velocity")
    ...
    v_tnd[1:-1, 1:-1, :] = -(mtg[1:-1, 2:, :] - mtg[1:-1, :-2, :]) / (2.0 * dy)
    v_tnd_val = get_dataarray_3d(v_tnd, grid, "m s^-2", name="y_velocity")
    ...
    nb = (
        2
    )  # TODO: nb = 2 if domain.horizontal_boundary.nb < 2 else domain.horizontal_boundary.nb
    u_tnd[2:-2, 2:-2, :] = -(
        mtg[:-4, 2:-2, :]
        - 8.0 * mtg[1:-3, 2:-2, :]
        + 8.0 * mtg[3:-1, 2:-2, :]
    ...
    u_tnd_val = get_dataarray_3d(u_tnd_val, grid, "m s^-2")
    v_tnd_val = get_dataarray_3d(v_tnd_val, grid, "m s^-2")
...
def test_conservative(data):
    ...
    su_tnd[1:-1, 1:-1, :] = (
        -s[1:-1, 1:-1, :] * (mtg[2:, 1:-1, :] - mtg[:-2, 1:-1, :]) / (2.0 * dx)
    )
    su_tnd_val = get_dataarray_3d(
    ...
    sv_tnd[1:-1, 1:-1, :] = (
        -s[1:-1, 1:-1, :] * (mtg[1:-1, 2:, :] - mtg[1:-1, :-2, :]) / (2.0 * dy)
    )
    sv_tnd_val = get_dataarray_3d(
    ...
    u_tnd_val, v_tnd_val = pressure_thickness_weighted_validation(dx, dy, pg._eps, p, mtg)
    su_tnd_val = get_dataarray_3d(s * u_tnd_val, grid, "kg m^-1 K^-1 s^-2")
    sv_tnd_val = get_dataarray_3d(s * v_tnd_val, grid, "kg m^-1 K^-1 s^-2")
