...
def get_third_order_upwind_fluxes(dx, dy, u, v, h):
    ...
    flux_x = np.zeros(u.shape, dtype=dtype)
    flux_x[2:-2, 2:-2, 1:-1] = slice_u / (12.0 * dx) * (
    ...
    flux_y = np.zeros(u.shape, dtype=dtype)
    flux_y[2:-2, 2:-2, 1:-1] = slice_v / (12.0 * dy) * (
...
def get_fifth_order_upwind_fluxes(dx, dy, u, v, h):
    ...
    flux_x = np.zeros(u.shape, dtype=dtype)
    flux_x[3:-3, 3:-3, 1:-1] = slice_u / (60.0 * dx) * (
    ...
    flux_y = np.zeros(u.shape, dtype=dtype)
    flux_y[3:-3, 3:-3, 1:-1] = slice_v / (60.0 * dy) * (
...
def test_boussinesq_tendency(data):
    ...
    b = flux_x + flux_y
    b_val = get_dataarray_3d(b, grid, "m s^-1")
    assert "metric_term" in diagnostics
    compare_dataarrays(diagnostics["metric_term"], b_val, compare_coordinate_values=False)
    assert len(diagnostics) == 1

    # s
    s_tnd = s * (b[:, :, :-1] - b[:, :, 1:]) / (h[:, :, :-1] - h[:, :, 1:])
    s_tnd_val = get_dataarray_3d(s_tnd, grid, "kg m^-2 K^-1 s^-1")
    assert "air_isentropic_density" in tendencies
    compare_dataarrays(
        tendencies["air_isentropic_density"], s_tnd_val, compare_coordinate_values=False
    )

    # ddmtg
    ddmtg = state["dd_montgomery_potential"].to_units("m^2 K^-2 s^-2").values
    ddmtg_tnd = ddmtg * (b[:, :, :-1] - b[:, :, 1:]) / (h[:, :, :-1] - h[:, :, 1:])
    ddmtg_tnd_val = get_dataarray_3d(ddmtg_tnd, grid, "m^2 K^-2 s^-3")
    assert "dd_montgomery_potential" in tendencies
    compare_dataarrays(
        tendencies["dd_montgomery_potential"],
        ddmtg_tnd_val,
    ...
    su_tnd_val = get_dataarray_3d(su_tnd, grid, "kg m^-1 K^-1 s^-2")
    assert "x_momentum_isentropic" in tendencies
    compare_dataarrays(
        tendencies["x_momentum_isentropic"], su_tnd_val, compare_coordinate_values=False
    )

    # sv
    sv_tnd = sv * (b[:, :, :-1] - b[:, :, 1:]) / (h[:, :, :-1] - h[:, :, 1:])
    sv_tnd_val = get_dataarray_3d(sv_tnd, grid, "kg m^-1 K^-1 s^-2")
    assert "y_momentum_isentropic" in tendencies
    compare_dataarrays(
        tendencies["y_momentum_isentropic"], sv_tnd_val, compare_coordinate_values=False
    )
    ...
    if moist:
        ...
        for name in names:
            ...
            q_tnd_val = get_dataarray_3d(q_tnd, grid, "g g^-1 s^-1")
