...
@pytest.mark.mpl_image_compare(baseline_dir='baseline_images/test_contour_xz')
def test_contour_xz_velocity(isentropic_dry_data):
	...
	save_dest = os.path.join(baseline_dir, 'test_contour_xz_velocity.eps')
...
@pytest.mark.mpl_image_compare(baseline_dir='baseline_images/test_contour_xz')
def test_contour_xz_cloud_liquid_water(isentropic_moist_data):
	# Field to plot
	...
	# Make sure the baseline image will exist at the end of this run
	save_dest = os.path.join(baseline_dir, 'test_contour_xz_cloud_liquid_water.eps')
	if os.path.exists(save_dest):
		os.remove(save_dest)
