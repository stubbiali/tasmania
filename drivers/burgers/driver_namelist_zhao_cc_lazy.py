...
# the object handling the Dirichlet boundary conditions
hb = taz.HorizontalBoundary.factory(nl.hb_type, nl.nx, nl.ny, nl.nb, **nl.hb_kwargs)
...
diff = taz.BurgersHorizontalDiffusion(
    domain, "computational", nl.diffusion_type, nl.diffusion_coeff, nl.backend, nl.dtype
)
...
for i in range(nt):
    ...
    # enforce the horizontal boundary conditions
    hb.enforce(state, field_names=("x_velocity", "y_velocity"), grid=cgrid)
