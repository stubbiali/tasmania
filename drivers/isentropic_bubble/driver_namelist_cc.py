...
state = taz.get_isentropic_state_from_temperature(
    ...
    nl.x_velocity,
    nl.y_velocity,
    nl.background_temperature,
    bubble_center_x=nl.bubble_center_x,
    bubble_center_y=nl.bubble_center_y,
    bubble_center_height=nl.bubble_center_height,
    bubble_radius=nl.bubble_radius,
    ...
    if (nl.print_frequency > 0) and ((i + 1) % nl.print_frequency == 0):
        ...
        print(
            "Iteration {:6d}: Tmax = {:8.4f} K, Tmin = {:8.4f} K".format(
                i + 1,
                state["air_temperature"].to_units("K").values.max(),
