...
dv = taz.IsentropicDiagnostics(
    domain, grid_type="numerical", moist=True, pt=pt, backend=nl.backend, dtype=nl.dtype
)
...
args_before_dynamics.append(
    {"component": pg, "time_integrator": "forward_euler", "substeps": 1}
)
...
if nl.rain_evaporation:
    ...
    args_before_dynamics.append(
        {"component": vf, "time_integrator": "rk3ws", "substeps": 1}
    )
    args_after_dynamics.append(
        {"component": vf, "time_integrator": "rk3ws", "substeps": 1}
    )
...
ap = taz.AccumulatedPrecipitation(domain, "numerical", backend=nl.backend, dtype=nl.dtype)
...
for i in range(nt):
    ...
    # compute the physics
    physics_after_dynamics(state, 0.5 * dt)
