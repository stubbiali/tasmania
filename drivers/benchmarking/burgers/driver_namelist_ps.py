...
def main(backend=None, namelist="namelist_ps.py", no_log=False):
    ...
    for i in range(nt):
        ...
        taz.Timer.start("dynamics")
        state_prv = dycore(state, {}, dt)
        taz.Timer.stop("dynamics")

        # calculate the physics
        taz.Timer.start("physics")
        physics(state, state_prv, dt)
        taz.Timer.stop("physics")

        # update the state
        taz.Timer.start("state_update")
        dict_op.copy(state, state_prv)
        state["time"] = nl.init_time + (i + 1) * dt
        taz.Timer.stop("state_update")

        # stop timing
        taz.Timer.stop("compute_time")
