...
def main(backend=None, namelist="namelist_ps.py", no_log=False):
    ...
    feed_module(target=nl, source=namelist_ps)
    ...
    args.append(TimeIntegrationOptions(component=idv))
    ...
    # component downgrading tendency_of_air_potential_temperature to
    # tendency variable
    ...
    args.append(
        TimeIntegrationOptions(
            component=ConcurrentCoupling(rfv, sd),
            scheme="rk3ws",
            substeps=1,
            enable_checks=nl.enable_checks,
            backend=nl.backend,
    ...
    if not no_log:
        # save to file
        exec_info_to_csv(nl.exec_info_csv, "ps", nl.backend, nl.bo)
        run_info_to_csv(nl.run_info_csv, "ps", nl.backend)
