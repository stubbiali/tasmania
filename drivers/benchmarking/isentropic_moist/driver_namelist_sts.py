...
def main(backend=None, namelist="namelist_sts.py", no_log=False):
    ...
    feed_module(target=nl, source=namelist_sts)
    ...
    physics = SequentialTendencySplitting(*args)
    ...
    if not no_log:
        # save to file
        exec_info_to_csv(nl.exec_info_csv, "sts", nl.backend, nl.bo)
        run_info_to_csv(nl.run_info_csv, "sts", nl.backend)
