...
class Periodic(HorizontalBoundary):
    ...
    def get_numerical_field(self, field, field_name=None):
        ...
        mx = nx + 1 if "at_u_locations" in field_name or "at_uv_locations" in field_name else nx
        mi = mx + 2 * nb
        my = ny + 1 if "at_v_locations" in field_name or "at_uv_locations" in field_name else ny
        ...
        try:
            ...
            li, lj = field.shape
            trg = np.zeros((li + 2 * nb, lj + 2 * nb), dtype=self.storage_options.dtype)
            src = to_numpy(field)
        ...
        trg[:mi, my + nb : my + 2 * nb] = (
            trg[:mi, nb + 1 : 2 * nb + 1] if my == ny else trg[:mi, nb + 2 : 2 * nb + 2]
        )
    ...
    def enforce_field(self, field, field_name=None, field_units=None, time=None):
        ...
        field[:mi, my + nb : my + 2 * nb] = (
            field[:mi, nb + 1 : 2 * nb + 1] if my == ny else field[:mi, nb + 2 : 2 * nb + 2]
        )
...
class Periodic1DX(HorizontalBoundary):
    ...
    def get_numerical_field(self, field, field_name=None):
        ...
        mx = nx + 1 if "at_u_locations" in field_name or "at_uv_locations" in field_name else nx
        mi = mx + 2 * nb
        my = 2 if "at_v_locations" in field_name or "at_uv_locations" in field_name else 1
...
class Periodic1DY(HorizontalBoundary):
    ...
    def get_numerical_field(self, field, field_name=None):
        ...
        field_name = field_name or ""
        mx = 2 if "at_u_locations" in field_name or "at_uv_locations" in field_name else 1
        my = ny + 1 if "at_v_locations" in field_name or "at_uv_locations" in field_name else ny
        mj = my + 2 * nb
