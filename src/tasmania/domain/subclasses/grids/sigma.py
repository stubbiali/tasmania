...
from tasmania.utils.utils import equal_to as eq, smaller_than as lt, smaller_or_equal_than as le
...
class Sigma3d(Grid):
    ...
    def __init__(
        ...
        physical_constants : `dict[str, sympl.DataArray]`, optional
            ...
            `tasmania.domain.sigma._d_physical_constants`
        ...
        height_interface_v = (
            Rd / g * np.log(1 / z_interface_v) * (T_sl - 0.5 * beta * np.log(1 / z_interface_v))
        )
        self.height_interface = sympl.DataArray(height_interface_v, attrs=self.z_interface.attrs)
    ...
    def _update_metric_terms(self):
        ...
        if eq(beta, 0.0):
            ...
            p0_s = p_sl * np.exp(
                -T_sl / beta * (1.0 - np.sqrt(1.0 - 2.0 * beta * g * hs / (Rd * T_sl * T_sl)))
            )

        # Reference pressure at the half levels
        a = p_sl * zv * (np.logical_and(le(zt, zv), le(zv, zf))) + p_sl * zf * (1.0 - zv) / (
            1.0 - zf
        ...
        if eq(beta, 0.0):
            ...
            z_hl = Rd / g * np.log(p_sl / p0_hl) * (T_sl - 0.5 * beta * np.log(p_sl / p0_hl))
