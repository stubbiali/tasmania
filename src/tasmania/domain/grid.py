...
class PhysicalGrid(Grid):
    ...
    def __init__(
        ...
        *,
        ...
        # xy-grid
        grid_xy = PhysicalHorizontalGrid(domain_x, nx, domain_y, ny, storage_options=so)
        ...
        if lt(values_z[0], values_z[1]):
            if not (le(values_z[0], zi_v) and le(zi_v, values_z[1])):
                raise ValueError(
                    f"z_interface should be in the range " f"({values_z[0]}, {values_z[1]})."
                )
        else:
            if not (le(values_z[1], zi_v) and le(zi_v, values_z[0])):
                raise ValueError(
                    f"z_interface should be in the range " f"({values_z[1]}, {values_z[0]})."
                )
