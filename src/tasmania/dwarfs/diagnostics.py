...
class HorizontalVelocity(GridComponent, StencilFactory):
    ...
    @staticmethod
    @stencil_definition(backend=("numpy", "cupy", "numba:cpu:numpy"), stencil="momenta")
    ...
    @staticmethod
    @stencil_definition(backend=("numpy", "cupy", "numba:cpu:numpy"), stencil="velocity_x")
    def _diagnose_velocity_x_numpy(
        ...
        *,
        ...
        if staggering:  # compile-time if
            out_u[i, j, k] = (in_du[im1, j, k] + in_du[i, j, k]) / (in_d[im1, j, k] + in_d[i, j, k])
        else:
    ...
    @staticmethod
    @stencil_definition(backend=("numpy", "cupy", "numba:cpu:numpy"), stencil="velocity_y")
    def _diagnose_velocity_y_numpy(
        ...
        *,
        ...
        if staggering:  # compile-time if
            out_v[i, j, k] = (in_dv[i, jm1, k] + in_dv[i, j, k]) / (in_d[i, jm1, k] + in_d[i, j, k])
        else:
...
class WaterConstituent(GridComponent, StencilFactory):
    ...
    @staticmethod
    @stencil_definition(backend=("numpy", "cupy", "numba:cpu:numpy"), stencil="density")
    ...
    @staticmethod
    @stencil_definition(backend=("numpy", "cupy", "numba:cpu:numpy"), stencil="mass_fraction")
