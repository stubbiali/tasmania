...
class FirstOrder(HorizontalSmoothing):
    ...
    @staticmethod
    @stencil_definition(backend=("numpy", "cupy", "numba:cpu:numpy"), stencil="smoothing")
    ...
    def _smoothing_gt4py(
        ...
        with computation(PARALLEL), interval(...):
            out_phi = (1.0 - in_gamma[0, 0, 0]) * in_phi[0, 0, 0] + 0.25 * in_gamma[0, 0, 0] * (
...
class FirstOrder1DX(HorizontalSmoothing):
    ...
    def _smoothing_numpy(in_phi, in_gamma, out_phi, *, origin, domain):
        ...
        out_phi[i, j, k] = (1.0 - 0.5 * in_gamma[i, j, k]) * in_phi[i, j, k] + 0.25 * in_gamma[
            i, j, k
        ] * (in_phi[im1, j, k] + in_phi[ip1, j, k])
