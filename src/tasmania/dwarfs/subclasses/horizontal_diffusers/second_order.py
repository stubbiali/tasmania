...
class SecondOrder(HorizontalDiffusion):
    ...
    @staticmethod
    @stencil_definition(backend=("numpy", "cupy", "numba:cpu:numpy"), stencil="diffusion")
    def _diffusion_numpy(in_phi, in_gamma, out_phi, *, dx, dy, ow_out_phi, origin, domain):
    ...
    def _diffusion_numba_cpu(in_phi, in_gamma, out_phi, *, dx, dy, ow_out_phi, origin, domain):
...
class SecondOrder1DX(HorizontalDiffusion):
    ...
    def _diffusion_numpy(in_phi, in_gamma, out_phi, *, dx, dy, ow_out_phi, origin, domain):
        ...
        tmp = in_gamma[i, j] * (in_phi[im1, j] - 2.0 * in_phi[i, j] + in_phi[ip1, j]) / (dx * dx)
        set_output(out_phi[i, j], tmp, ow_out_phi)
    ...
    @staticmethod
    @stencil_definition(backend="numba:cpu:stencil", stencil="diffusion")
    def _diffusion_numba_cpu(in_phi, in_gamma, out_phi, *, dx, dy=0.0, ow_out_phi, origin, domain):
...
class SecondOrder1DY(HorizontalDiffusion):
    ...
    def _diffusion_numpy(in_phi, in_gamma, out_phi, *, dx, dy, ow_out_phi, origin, domain):
        ...
        tmp = in_gamma[i, j] * (in_phi[i, jm1] - 2.0 * in_phi[i, j] + in_phi[i, jp1]) / (dy * dy)
        set_output(out_phi[i, j], tmp, ow_out_phi)
    ...
    @staticmethod
    @stencil_definition(backend="numba:cpu:stencil", stencil="diffusion")
    def _diffusion_numba_cpu(in_phi, in_gamma, out_phi, *, dx=0.0, dy, ow_out_phi, origin, domain):
