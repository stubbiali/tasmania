    ...
    def __new__(cls, backend: str, stencil: str) -> Callable:
        ...
        try:
            ...
            raise FactoryRegistryError(
                f"No subroutine '{stencil}' has been registered " f"for the backend '{backend}'."
            )
...
class Compiler:
    ...
    def __new__(
        ...
        try:
            ...
            raise FactoryRegistryError(f"No compiler registered for the backend '{backend}'.")
        ...
        *,
        ...
        try:
            ...
            try:
                ...
                raise FactoryRegistryError(
                    f"No storage converter registered for the backend " f"'{backend}'."
                )
    ...
    def get_stencil_compiler(self, backend: str, stencil: Optional[str] = None):
        ...
        try:
            ...
            try:
                ...
                raise FactoryRegistryError(f"No compiler found for the backend '{backend}'.")
    ...
    def _fill_registry(self) -> None:
        ...
        for name, handle in methods:
            if getattr(handle, prt.attribute, None) is not None:
                ...
                args = tuple(item for pair in prt_dict.items() for item in pair)
