...
from sympl._core.storage import get_arrays_with_properties, restore_data_arrays_with_properties
...
if TYPE_CHECKING:
    ...
    from tasmania.utils.typingx import DataArrayDict, NDArrayDict, PropertyDict
...
class FakeComponent:
    def __init__(self, properties: dict[str, PropertyDict]) -> None:
...
class OfflineDiagnosticComponent(abc.ABC):
    ...
    def __init__(self) -> None:
        assert isinstance(
            self.input_properties, SequenceType
        ), "input_properties attribute is of type {}, " "but should be a sequence of dict.".format(
        ...
        assert isinstance(
            self.diagnostic_properties, dict
        ), "diagnostic_properties attribute is of type {}, " "but should be a dict.".format(
    ...
    @property
    @abc.abstractmethod
    def input_properties(self) -> Sequence[PropertyDict]:
        """
        Returns
        -------
    ...
    def __call__(self, *states: DataArrayDict) -> DataArrayDict:
        """
        ...
        for input_checker, state in zip(self._input_checkers, states):
            input_checker.check(state)

        raw_states = []
        for input_properties, state in zip(self.input_properties, states):
            raw_states.append(get_arrays_with_properties(state, input_properties))
        ...
        self._diagnostic_checker.check(raw_diagnostics_without_time)
    ...
    @abc.abstractmethod
    def array_call(self, *states: NDArrayDict) -> NDArrayDict:
        """
...
class RMSD(OfflineDiagnosticComponent):
    ...
    @property
    def input_properties(self) -> tuple[PropertyDict, PropertyDict]:
        g1, g2 = self._grids
    ...
    def array_call(self, state1: NDArrayDict, state2: NDArrayDict) -> NDArrayDict:
...
class ColumnSum(OfflineDiagnosticComponent):
    ...
    @property
    def input_properties(self) -> tuple[PropertyDict]:
        g = self._grid
    ...
    def array_call(self, state: NDArrayDict) -> NDArrayDict:
