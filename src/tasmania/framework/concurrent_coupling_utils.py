...
class StaticOperator:
    ...
    def get_overwrite_tendencies(cls, coupler: ConcurrentCoupling) -> list[dict[str, bool]]:
        ...
        for component in coupler.components:
            tendency_properties = cls.tendency_operator.get_properties(component)
    ...
    def _get_input_properties(
        ...
        for component in components:
            ...
            input_units = {name: input_properties[name]["units"] for name in input_properties}
            input_aliases = cls.input_operator.get_aliases(component)
            for name in input_properties:
                if name in out:
                    out[name]["dims"] = merge_dims(out[name]["dims"], input_dims[name])
    ...
    def _get_diagnostic_properties(cls, components: Sequence[Component]) -> PropertyDict:
        ...
        for component in components:
            ...
            diagnostic_units = {
                name: diagnostic_properties[name]["units"] for name in diagnostic_properties
            }
    ...
    def _get_tendency_properties(cls, components: Sequence[Component]) -> PropertyDict:
        ...
        for component in components:
            ...
            tendency_units = {
                name: tendency_properties[name]["units"] for name in tendency_properties
            }
            tendency_aliases = cls.tendency_operator.get_aliases(component)
            for name in tendency_properties:
                if name in out:
                    out[name]["dims"] = merge_dims(out[name]["dims"], tendency_dims[name])
...
class StaticChecker:
        ...
        for component in components:
            ...
            for name in input_dims:
                if name in dims:
                    try:
                        ...
                        raise CouplingError(
                            f"ConcurrentCoupling, " f"{component.__class__.__name__}: {err}."
                        )
            ...
            if not ignore_diagnostics:
                ...
                for name in diagnostic_dims:
                    if name in dims:
                        try:
                            check_dims_are_compatible(dims[name], diagnostic_dims[name])
                        except IncompatibleDimensionsError as err:
                            raise CouplingError(
                                f"ConcurrentCoupling, " f"{component.__class__.__name__}: {err}."
                            )
    ...
    def check_input_units(cls, components: Sequence[Component], ignore_diagnostics: bool) -> None:
        ...
        for component in components:
            ...
            for name in input_properties:
                if name in units:
                    try:
                        check_units_are_compatible(units[name], input_properties[name]["units"])
            ...
            if not ignore_diagnostics:
                diagnostic_properties = cls.so.diagnostic_operator.get_properties(component)
                for name in diagnostic_properties:
                    if name in units:
                        try:
        ...
        for component in components:
            ...
            for name in diagnostic_dims:
                ...
                if name in input_properties:
                    try:
                        check_dims_are_compatible(dims[name], input_properties[name]["dims"])
                    except IncompatibleDimensionsError as err:
                        raise CouplingError(
                            f"ConcurrentCoupling, " f"{component.__class__.__name__}: {err}."
                        )

                    dims[name] = merge_dims(dims[name], input_properties[name]["dims"])
    ...
    def check_tendency_dims(cls, components: Sequence[Component], ignore_diagnostics: bool) -> None:
        ...
        for component in components:
            ...
            for name in tendency_dims:
                if name in dims:
                    try:
                        check_dims_are_compatible(dims[name], tendency_dims[name])
                    except IncompatibleDimensionsError as err:
                        raise CouplingError(
                            f"ConcurrentCoupling, " f"{component.__class__.__name__}: {err}."
                        )
                ...
                if name in diagnostic_properties:
                    try:
                        check_dims_are_compatible(dims[name], diagnostic_properties[name]["dims"])
                    except IncompatibleDimensionsError as err:
                        raise CouplingError(
                            f"ConcurrentCoupling, " f"{component.__class__.__name__}: {err}."
                        )

                    dims[name] = merge_dims(dims[name], diagnostic_properties[name]["dims"])
    ...
    def check_tendency_units(
        ...
        for component in components:
            tendency_properties = cls.so.tendency_operator.get_properties(component)
            for name in tendency_properties:
                if name in units:
                    try:
                        check_units_are_compatible(units[name], tendency_properties[name]["units"])
                    except IncompatibleUnitsError as err:
                        raise CouplingError(
                            f"ConcurrentCoupling, " f"{component.__class__.__name__}: {err}."
                        )
    ...
    def check_t2d(cls, components: Sequence[Component]) -> None:
        ...
        for component in components:
            if isinstance(component, BaseFromTendencyToDiagnostic):
                ...
                missing = [key for key in requirements if key not in tendencies]
                if len(missing) > 0:
                    raise CouplingError()
...
class DynamicOperator:
    ...
    def allocate_tendencies(self, state: DataArrayDict) -> DataArrayDict:
        ...
        for component in self.coupler.components:
            ...
            if allocate_tendency is not None:
                for name in tendency_properties:
                    if name in self.coupler.tendency_properties and name not in raw_tendencies:
                        raw_tendencies[name] = allocate_tendency(name)
                        raw_tendencies[name][...] = 0.0

        tendencies = self.dco_tendencies.get_dataarray_dict(raw_tendencies, state)
    ...
    def allocate_diagnostics(self, state: DataArrayDict) -> DataArrayDict:
        ...
        for component in self.coupler.components:
            ...
            if allocate_diagnostic is not None:
                for name in diagnostic_properties:
                    if name in self.coupler.diagnostic_properties and name not in raw_diagnostics:
                        raw_diagnostics[name] = allocate_diagnostic(name)
                        raw_diagnostics[name][...] = 0.0

        diagnostics = self.dco_diagnostics.get_dataarray_dict(raw_diagnostics, state)
            ...
            if allocate_diagnostic is not None:
                for name in diagnostic_properties:
                    if occurrences[name] > 1:
                        raw_internal_diagnostics[name] = allocate_diagnostic(name)
