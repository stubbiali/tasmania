...
class ParallelSplitting:
    ...
    def __init__(
        ...
        *args : TimeIntegrationOptions
        ...
        self.provisional_input_properties = StaticOperator.get_provisional_input_properties(self)
        self.output_properties = StaticOperator.get_output_properties(self)
        self.provisional_output_properties = StaticOperator.get_provisional_output_properties(self)

        # static checks
        check_properties_are_compatible(self, "input_properties", self, "output_properties")
        check_properties_are_compatible(
            self,
    ...
    def _call_serial(
        ...
        for idx, component in enumerate(self.components):
            if not isinstance(component, self.allowed_diagnostic_type):
                ...
                try:
                    self._out_diagnostics[idx] = component(arg, out=self._out_diagnostics[idx])
                except TypeError:
    ...
    def _call_asparallel(
        ...
        for component, substeps in zip(self.components, self._substeps):
            if not isinstance(component, self.allowed_diagnostic_type):
                ...
                if substeps > 1:
                    state_tmp.update(
                        {key: value for key, value in state.items() if key not in state_tmp}
                    )
