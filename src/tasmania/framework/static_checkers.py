...
def check_dims_are_compatible(dim1: Sequence[str], dim2: Sequence[str]) -> None:
    if "*" not in dim1 and "*" not in dim2:
        ...
        if any(d not in dim2 for d in dim1) or any(d not in dim1 for d in dim2):
            raise IncompatibleDimensionsError(dim1, dim2)
...
def _check_properties_are_compatible(
    ...
    for key in shared_keys:
        ...
        if "units" in prop1 and "units" in prop2:
            check_units_are_compatible(prop1["units"] + units_suffix, prop2["units"])
...
def check_properties_are_compatible(
    ...
    try:
        _check_properties_are_compatible(properties, other_properties, units_suffix)
