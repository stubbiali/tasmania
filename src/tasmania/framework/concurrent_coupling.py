...
class ConcurrentCoupling(BaseConcurrentCoupling):
    ...
    allowed_component_type = allowed_diagnostic_type + allowed_tendency_type + allowed_promoter_type
    ...
    def __init__(
        ...
        *args : obj
        ...
        if not getattr(self, "_initialized", False):
            ...
            self._policy = (
                execution_policy if execution_policy in ("serial", "as_parallel") else "serial"
            )
            ...
            self.diagnostic_properties = StaticOperator.get_diagnostic_properties(self)
            ...
            self.overwrite_tendencies = StaticOperator.get_overwrite_tendencies(self)

            # retrieve the object handling the horizontal boundary
            self.horizontal_boundary = StaticOperator.get_horizontal_boundary(self)
    ...
    def __call__(
        ...
        *,
        ...
        out_diagnostics = out_diagnostics if out_diagnostics is not None else {}
        overwrite_tendencies = overwrite_tendencies if overwrite_tendencies is not None else {}
    ...
    def _call_serial(
        ...
        for component, self_overwrite_tendencies in zip(self.components, self.overwrite_tendencies):
            if isinstance(component, self.allowed_diagnostic_type):
                ...
                ot = {
                    name: self_overwrite_tendencies[name] and overwrite_tendencies.get(name, True)
            ...
            sub_diagnostics = {
                name: out_diagnostics[name] for name in sco.get_properties(component)
            }
