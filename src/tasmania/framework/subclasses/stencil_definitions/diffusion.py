...
def diffusion_numpy(in_phi, out_phi, *, alpha, origin, domain):
    ib, jb, kb = origin
    ...
    bilap = -4 * lap[1:-1, 1:-1] + lap[:-2, 1:-1] + lap[2:, 1:-1] + lap[1:-1, :-2] + lap[1:-1, 2:]
...
if numba:
    stencil_definition.register(diffusion_numpy, "numba:cpu:numpy", "diffusion")
