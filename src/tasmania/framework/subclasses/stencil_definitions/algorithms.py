...
def irelax_numpy(in_gamma, in_phi_ref, inout_phi, *, origin, domain):
    ...
    inout_phi[idx] = np.where(
        ...
        np.where(
            in_gamma[idx] == 1.0,
            in_phi_ref[idx],
            inout_phi[idx] - in_gamma[idx] * (inout_phi[idx] - in_phi_ref[idx]),
        ),
    )
...
def sts_rk2_0_numpy(in_field, in_field_prv, in_tnd, out_field, *, dt, origin, domain):
    idx = tuple(slice(o, o + d) for o, d in zip(origin, domain))
    out_field[idx] = 0.5 * (in_field[idx] + in_field_prv[idx] + dt * in_tnd[idx])


@stencil_definition.register(backend="numpy", stencil="sts_rk3ws_0")
def sts_rk3ws_0_numpy(in_field, in_field_prv, in_tnd, out_field, *, dt, origin, domain, **kwargs):
    idx = tuple(slice(o, o + d) for o, d in zip(origin, domain))
...
if numba:
    ...
    stencil_definition.register(sts_rk2_0_numpy, "numba:cpu:numpy", "sts_rk2_0")
    stencil_definition.register(sts_rk3ws_0_numpy, "numba:cpu:numpy", "sts_rk3ws_0")
    ...
    def irelax_numba_cpu(in_gamma, in_phi_ref, inout_phi, *, origin, domain):
        def core_def(gamma, phi, phi_ref):
            if gamma[0, 0, 0] == 0.0:
                ...
                return phi[0, 0, 0] - gamma[0, 0, 0] * (phi[0, 0, 0] - phi_ref[0, 0, 0])
    ...
    def relax_numba_cpu(in_gamma, in_phi, in_phi_ref, out_phi, *, origin, domain):
        def core_def(gamma, phi, phi_ref):
            if gamma[0, 0, 0] == 0.0:
                return phi[0, 0, 0]
            elif gamma[0, 0, 0] == 1.0:
    ...
    @stencil_definition.register(backend="numba:cpu:stencil", stencil="sts_rk2_0")
    def sts_rk2_0_numba_cpu(in_field, in_field_prv, in_tnd, out_field, *, dt, origin, domain):
        def core_def(field, field_prv, field_tnd, dt):
            return 0.5 * (field[0, 0, 0] + field_prv[0, 0, 0] + dt * field_tnd[0, 0, 0])
    ...
    @stencil_definition.register(backend="numba:cpu:stencil", stencil="sts_rk3ws_0")
    def sts_rk3ws_0_numba_cpu(in_field, in_field_prv, in_tnd, out_field, *, dt, origin, domain):
        def core_def(field, field_prv, field_tnd, dt):
            return (2.0 * field[0, 0, 0] + field_prv[0, 0, 0] + dt * field_tnd[0, 0, 0]) / 3.0
