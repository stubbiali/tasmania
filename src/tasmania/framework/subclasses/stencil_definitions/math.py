...
def fma_numpy(in_a, in_b, out_c, *, f, origin, domain):
    ib, jb, kb = origin
    ie, je, ke = ib + domain[0], jb + domain[1], kb + domain[2]
    out_c[ib:ie, jb:je, kb:ke] = in_a[ib:ie, jb:je, kb:ke] + f * in_b[ib:ie, jb:je, kb:ke]
...
def iaddsub_numpy(inout_a, in_b, in_c, *, origin, domain):
    ib, jb, kb = origin
    ie, je, ke = ib + domain[0], jb + domain[1], kb + domain[2]
    inout_a[ib:ie, jb:je, kb:ke] += in_b[ib:ie, jb:je, kb:ke] - in_c[ib:ie, jb:je, kb:ke]
...
if numba:
    ...
    @stencil_definition.register(backend="numba:cpu:stencil", stencil="iaddsub")
