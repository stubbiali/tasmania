        ...
        if field_name in state:  # model variable
            ...
            try:
                ...
                try:
                    ...
                    su = (
                        state["x_momentum_isentropic"][x, y, z].to_units("kg m^-1 K^-1 s^-1").values
                    )
                    sv = (
                        state["y_momentum_isentropic"][x, y, z].to_units("kg m^-1 K^-1 s^-1").values
                    )
                    u, v = su / s, sv / s
                except KeyError:
                    try:
            ...
            factor = (
                ...
                else DataArray(1, attrs={"units": "m s^-1"}).to_units(field_units).values.item()
            )
...
class DataRetrieverComposite:
    ...
    def __init__(
        ...
        if isinstance(field_name, str):
            fnames = ((field_name,),)
        elif isinstance(field_name, SequenceType) and all(
            isinstance(name, str) for name in field_name
        ):
            fnames = (field_name,)
        elif isinstance(field_name, SequenceType) and all(
            isinstance(name, SequenceType) for name in field_name
        ):
            fnames = field_name
        else:
            raise TypeError(
        ...
        elif isinstance(field_units, SequenceType) and all(
            isinstance(unit, str) for unit in field_units
        ):
            funits = (field_units,)
        elif isinstance(field_units, SequenceType) and all(
            isinstance(unit, SequenceType) for unit in field_units
        ):
            funits = field_units
        else:
            raise TypeError(
        ...
        assert len(funits) == len(fnames), "field_units''s length: expected {}, got{}.".format(
        ...
        if x is None:
            ...
            fx = (x,)
        elif isinstance(x, SequenceType) and all(isinstance(arg, SequenceType) for arg in x):
            fx = x
        else:
            raise TypeError(
        ...
        for i in range(len(fx)):
            assert len(fx[i]) == len(fnames[i]), "x[{}]''s length: expected {}, got{}.".format(
        ...
        if y is None:
            ...
            fy = (y,)
        elif isinstance(y, SequenceType) and all(isinstance(arg, SequenceType) for arg in y):
            fy = y
        else:
            raise TypeError(
        ...
        for i in range(len(fy)):
            assert len(fy[i]) == len(fnames[i]), "y[{}]''s length: expected {}, got{}.".format(
        ...
        if z is None:
            ...
            fz = (z,)
        elif isinstance(z, SequenceType) and all(isinstance(arg, SequenceType) for arg in z):
            fz = z
        else:
            raise TypeError(
        ...
        for i in range(len(fy)):
            assert len(fz[i]) == len(fnames[i]), "z[{}]''s length: expected {}, got{}.".format(
    ...
    def __call__(
        self, *args: Union[typingx.DataArrayDict, Sequence[typingx.DataArrayDict]]
