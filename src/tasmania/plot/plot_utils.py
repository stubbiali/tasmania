    ...
    if cbar_ax is None:
        ...
        try:
            ...
            cb = fig.colorbar(
                ...
                extendfrac=cbar_extendfrac,
                extendrect=cbar_extendrect,
                ax=(axes[cbar_ax] if isinstance(cbar_ax, int) else [axes[i] for i in cbar_ax]),
            )
        except TypeError:
...
def make_contour(x: np.ndarray, y: np.ndarray, field: np.ndarray, ax: plt.Axes, **kwargs) -> None:
    """
...
def make_contourf(
    x: np.ndarray, y: np.ndarray, field: np.ndarray, fig: plt.Figure, ax: plt.Axes, **kwargs
) -> None:
    """
