...
class Contour(Drawer):
    ...
    def __init__(
        ...
        properties : `dict`, optional
            ...
            See :func:`~tasmania.python.plot.plot_utils.make_contour`.
        """
        ...
        if flag_z:
            ...
            if zaxis_name != "z":
                ...
                zaslice_x = (
                    None if zax is None else slice(zax, zax + 1 if zax != -1 else None, None)
                )
                zaslice_y = (
                    None if zay is None else slice(zay, zay + 1 if zay != -1 else None, None)
                )
                zaxis_retriever = DataRetriever(grid, zaxis_name, zaxis_units, zaslice_x, zaslice_y)

                if flag_x:
                    self._slave = lambda state, ax: make_contour_yh(
                        grid, yaxis_units, zaxis_retriever, retriever, state, ax, **self.properties
                    )
                else:
                    self._slave = lambda state, ax: make_contour_xh(
                        grid, xaxis_units, zaxis_retriever, retriever, state, ax, **self.properties
                    )
            else:
                if flag_x:
    ...
    def __call__(self, state: typingx.DataArrayDict, fig: plt.Figure, ax: plt.Axes) -> None:
        """
