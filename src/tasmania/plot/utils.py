...
def assert_sequence(
    ...
    if reflen is not None:
        assert len(seq) == reflen, "The input sequence has length {}, but {} was expected.".format(
            len(seq), reflen
        )
    ...
    if reftype is not None:
        ...
        for item in seq:
            error_msg = (
                "An item of the input sequence is of type " + str(type(item)) + ", but one of [ "
            )
...
def get_time_string(seconds: float) -> str:
    ...
    s += "0{}".format(int(remainder)) if int(remainder) < 10 else "{}".format(int(remainder))
