    ...
    if numba:
        ...
        def call_numba_cpu(dx, dy, u, v):
            ...
            def advection_x_def(u, abs_u, phi, dx):
                return u[0, 0, 0] / (2.0 * dx) * (phi[+1, 0, 0] - phi[-1, 0, 0]) - abs_u[
                    0, 0, 0
                ] / (2.0 * dx) * (phi[+1, 0, 0] - 2.0 * phi[0, 0, 0] + phi[-1, 0, 0])

            def advection_y_def(v, abs_v, phi, dy):
                return v[0, 0, 0] / (2.0 * dy) * (phi[0, +1, 0] - phi[0, -1, 0]) - abs_v[
                    0, 0, 0
                ] / (2.0 * dy) * (phi[0, +1, 0] - 2.0 * phi[0, 0, 0] + phi[0, -1, 0])
