...
class KesslerMicrophysics(TendencyComponent):
    ...
    def array_call(
        ...
        if self._air_pressure_on_interface_levels:
            ...
            stencil_args["in_exn"] = state["exner_function_on_interface_levels"]
        else:
    ...
    def _kessler_numpy(
        ...
        *,
        ...
        ps = 610.78 * np.exp(17.27 * (in_t[i, j, k] - 273.16) / (in_t[i, j, k] - 35.86))
        ...
        cr = k2 * in_qc[i, j, k] * np.where(in_qr[i, j, k] > 0.0, in_qr[i, j, k] ** 0.875, 0.0)
        ...
        if not rain_evaporation:
            ...
            set_output(out_theta_tnd[i, j, k], -lhvw / exn * er, ow_out_theta_tnd)
    ...
    def _saturation_diagnostic_gt4py(
        ...
        *,
        ...
        with computation(PARALLEL), interval(...):
            ...
            # calculate the tendency of air potential temperature
            tnd_theta = set_output(tnd_theta, (lhvw / exn) * (-dq / dt), ow_tnd_theta)
...
class KesslerSaturationAdjustmentPrognostic(TendencyComponent):
    ...
    def __init__(
        ...
        *,
        ...
        self._sr = (
            saturation_rate.to_units("s^-1").values.item() if saturation_rate is not None else 0.5
        )
    ...
    def _saturation_prognostic_gt4py(
        ...
        *,
        ...
        with computation(PARALLEL), interval(...):
            ...
            tnd_theta = set_output(tnd_theta, -sr * (lhvw / exn) * dq, ow_tnd_theta)
...
class KesslerFallVelocity(DiagnosticComponent):
    ...
    def _fall_velocity_numpy(
        ...
        *,
        ...
        out_vt[i, j, k] = (
            36.34
            * (1.0e-3 * in_rho[i, j, k] * np.where(in_qr[i, j, k] > 0.0, in_qr[i, j, k], 0.0))
            ** 0.1346
...
class KesslerSedimentation(ImplicitTendencyComponent):
    ...
    def __init__(
        ...
        **kwargs :
        ...
        sflux = SedimentationFlux.factory(sedimentation_flux_scheme, backend=backend)
        dtype = self.storage_options.dtype
        self.backend_options.dtypes = {"dtype": dtype}
        self.backend_options.externals = {
            "set_output": self.get_subroutine_definition("set_output"),
    ...
    def _sedimentation_gt4py(
        ...
        *,
        ...
        with computation(PARALLEL), interval(sflux_extent, None):
            ...
            out_tnd_qr = set_output(out_tnd_qr, out_dfdz / in_rho, ow_out_tnd_qr)
