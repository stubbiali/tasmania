...
class Smagorinsky2d(TendencyComponent):
    ...
    def _stencil_numpy(
        ...
        *,
        ...
        tmp_out_u_tnd, tmp_out_v_tnd = core(in_u, in_v, dx, dy, cs, ib, ie, jb, je, k)
    ...
    def _core_numpy(u, v, dx, dy, cs, ib, ie, jb, je, k):
        ...
        s01 = 0.5 * (
            (u[ib - 1 : ie + 1, jb : je + 2, k] - u[ib - 1 : ie + 1, jb - 2 : je, k]) / (2.0 * dy)
            + (v[ib : ie + 2, jb - 1 : je + 1, k] - v[ib - 2 : ie, jb - 1 : je + 1, k]) / (2.0 * dx)
        )
        s11 = (v[ib - 1 : ie + 1, jb : je + 2, k] - v[ib - 1 : ie + 1, jb - 2 : je, k]) / (2.0 * dy)
    ...
    def _core_gt4py(u, v, dx, dy, cs):
        ...
        s01 = 0.5 * (
            (u[0, +1, 0] - u[0, -1, 0]) / (2.0 * dy) + (v[+1, 0, 0] - v[-1, 0, 0]) / (2.0 * dx)
        )
