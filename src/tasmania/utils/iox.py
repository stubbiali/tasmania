...
class NetCDFMonitor(DomainComponent, StencilFactory, sympl.NetCDFMonitor):
    ...
    def __init__(
        ...
        *,
        ...
        super(GridComponent, self).__init__(backend=backend, storage_options=storage_options)
    ...
    def store(self, state: DataArrayDict) -> None:
        ...
        if self.grid_type == "physical":
            ...
            if not (state[names[0]].dims[0] in dims_x and state[names[0]].dims[1] in dims_y):
