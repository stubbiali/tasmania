...
def get_dataarray_2d(
    ...
    try:
        ...
        raise ValueError(f"Expected a 2-D array, got a {len(grid_shape)}-D one.")
...
def get_dataarray_3d(
    ...
    try:
        ...
        raise ValueError(f"Expected a 3-D array, got a {len(grid_shape)}-D one.")
...
def get_physical_state(
    ...
    store_names = (
        store_names if store_names is not None else tuple(name for name in cstate if name != "time")
    )
...
def get_numerical_state(
    ...
    store_names = (
        store_names if store_names is not None else tuple(name for name in pstate if name != "time")
    )
...
def get_storage_shape(
    ...
    if max_shape is None:
        ...
        assert all(tuple(out_shape[i] >= min_shape[i] for i in range(len(min_shape)))), error_msg
    else:
        error_msg = "storage shape must be between {} and {}".format(min_shape, max_shape)
        assert all(
            tuple(min_shape[i] <= out_shape[i] <= max_shape[i] for i in range(len(min_shape)))
        ), error_msg
...
def get_aligned_index(
    ...
    max_aligned_index = tuple(
        max_aligned_index[i] if storage_shape[i] > 2 * max_aligned_index[i] else 0 for i in range(3)
    )
    ...
    for name in src:
        if name != "time":
            if backend is not None:
                dst[name] = as_storage(backend, data=src[name], storage_options=storage_options)
            else:
