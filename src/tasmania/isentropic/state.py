...
def get_isentropic_state_from_brunt_vaisala_frequency(
    ...
    *,
    ...
    pcs = get_physical_constants(default_physical_constants, physical_constants)
    ...
    def allocate():
        return zeros(backend, shape=storage_shape, storage_options=storage_options)

    def allocate_numpy():
        return zeros("numpy", shape=storage_shape, storage_options=storage_options)
    ...
    exn_np[:nx, :ny, nz] = cp
    for k in range(nz - 1, -1, -1):
        exn_np[:nx, :ny, k : k + 1] = exn_np[:nx, :ny, k + 1 : k + 2] - dz * (g**2) / (
    ...
    p_np = allocate_numpy()
    p_np[:nx, :ny, : nz + 1] = pref * ((exn_np[:nx, :ny, : nz + 1] / cp) ** (cp / Rd))
    p = from_numpy(p_np)

    # diagnose the Montgomery potential
    mtg_s = (
        g * h_np[:, :, nz : nz + 1]
        + grid.z_on_interface_levels.to_units("K").values[-1] * exn_np[:, :, nz : nz + 1]
    )
    mtg_np = allocate_numpy()
    mtg_np[:nx, :ny, nz - 1] = mtg_s[:nx, :ny, 0] + 0.5 * dz * exn_np[:nx, :ny, nz]
    for k in range(nz - 2, -1, -1):
        mtg_np[:nx, :ny, k] = mtg_np[:nx, :ny, k + 1] + dz * exn_np[:nx, :ny, k + 1]
    mtg = from_numpy(mtg_np)

    # diagnose the isentropic density and the momenta
    s_np = allocate_numpy()
    s_np[:nx, :ny, :nz] = -(p_np[:nx, :ny, :nz] - p_np[:nx, :ny, 1 : nz + 1]) / (g * dz)
    s = from_numpy(s_np)
    su_np = allocate_numpy()
    su_np[:nx, :ny, :nz] = (
        0.5 * s_np[:nx, :ny, :nz] * (u_np[:nx, :ny, :nz] + u_np[1 : nx + 1, :ny, :nz])
    )
    su = from_numpy(su_np)
    sv_np = allocate_numpy()
    sv_np[:nx, :ny, :nz] = (
        0.5 * s_np[:nx, :ny, :nz] * (v_np[:nx, :ny, :nz] + v_np[:nx, 1 : ny + 1, :nz])
    )
    sv = from_numpy(sv_np)
    ...
    if moist:
        ...
        rho_np[:nx, :ny, :nz] = (
            s_np[:nx, :ny, :nz] * dz / (h_np[:nx, :ny, :nz] - h_np[:nx, :ny, 1 : nz + 1])
        )
        ...
        temp_np[:nx, :ny, :nz] = (
            0.5 * (exn_np[:nx, :ny, :nz] + exn_np[:nx, :ny, 1 : nz + 1]) * theta1d[:, :, :nz] / cp
        )
        ...
        p_unstg_np[:nx, :ny, :nz] = 0.5 * (p_np[:nx, :ny, :nz] + p_np[:nx, :ny, 1 : nz + 1])
...
def get_isentropic_state_from_temperature(
    ...
    *,
    ...
    p_np = allocate_numpy()
    p_np[:nx, :ny, : nz + 1] = pref * ((temp / theta1d[:, :, : nz + 1]) ** (cp / Rd))
    ...
    for k in range(nz - 1, -1, -1):
        h_np[:nx, :ny, k] = h_np[:nx, :ny, k + 1] - Rd / (cp * g) * (
            theta1d[:, :, k] * exn_np[:nx, :ny, k] + theta1d[:, :, k + 1] * exn_np[:nx, :ny, k + 1]
    ...
    if bubble_maximum_perturbation is not None:
        ...
        d = np.sqrt(((x - cx) ** 2 + (y - cy) ** 2 + (h_np - ch) ** 2) / r**2)
        t = allocate_numpy()
        t[:nx, :ny, : nz + 1] = temp + delta * (np.cos(0.5 * np.pi * d)) ** 2 * (d <= 1.0)
    else:
        t = allocate_numpy()
        t[:nx, :ny, : nz + 1] = temp
