...
if TYPE_CHECKING:
    ...
    from tasmania.utils.typingx import GTField, NDArray, NDArrayDict, PropertyDict, TripletInt
...
@gtscript.function
def first_order_boundary(dz: float, w: GTField, phi: GTField) -> GTField:
    out = (w[0, 0, -1] * phi[0, 0, -1] - w[0, 0, 0] * phi[0, 0, 0]) / dz
    return out


@gtscript.function
def second_order_boundary(dz: float, w: GTField, phi: GTField) -> GTField:
    out = (
        0.5
        * (
...
class IsentropicVerticalAdvection(TendencyComponent):
    ...
    def __init__(
        ...
        **kwargs :
        ...
        self._vflux = IsentropicMinimalVerticalFlux.factory(flux_scheme, backend=self.backend)
        ...
        self.backend_options.externals = {
            ...
            "flux_end": -self._vflux.extent + 1 if self._vflux.extent > 1 else None,
            "flux_extent": self._vflux.extent,
            "get_flux_dry": self._vflux.get_subroutine_definition("flux_dry"),
            "get_flux_moist": self._vflux.get_subroutine_definition("flux_moist"),
            "moist": moist,
    ...
    def _stencil_numpy(
        ...
        *,
        ...
        if self._moist:
            ...
            tmp_out_qv[i, j, kstart + nb : kstop - nb] = (fsqv[:, j, 1:] - fsqv[:, j, :-1]) / (
            ...
            tmp_out_qc[i, j, kstart + nb : kstop - nb] = (fsqc[:, :, 1:] - fsqc[:, :, :-1]) / (
                in_s[i, j, kstart + nb : kstop - nb] * dz
            )
            ...
            tmp_out_qr[i, j, kstart + nb : kstop - nb] = (fsqr[:, :, 1:] - fsqr[:, :, :-1]) / (
                in_s[i, j, kstart + nb : kstop - nb] * dz
            )
    ...
    def _stencil_gt4py(
        ...
        *,
        ...
        with computation(PARALLEL), interval(flux_extent, flux_end):
            flux_s, flux_su, flux_sv = get_flux_dry(dt=dt, dz=dz, w=w, s=in_s, su=in_su, sv=in_sv)
            if moist:
        ...
        with computation(PARALLEL), interval(flux_extent, -flux_extent):
            ...
            if moist:
                tmp_out_qv = (flux_sqv[0, 0, 1] - flux_sqv[0, 0, 0]) / (in_s[0, 0, 0] * dz)
                out_qv = set_output(out_qv, tmp_out_qv, ow_out_qv)
                tmp_out_qc = (flux_sqc[0, 0, 1] - flux_sqc[0, 0, 0]) / (in_s[0, 0, 0] * dz)
                out_qc = set_output(out_qc, tmp_out_qc, ow_out_qc)
                tmp_out_qr = (flux_sqr[0, 0, 1] - flux_sqr[0, 0, 0]) / (in_s[0, 0, 0] * dz)
...
class PrescribedSurfaceHeating(TendencyComponent):
    ...
    def __init__(
        ...
        **kwargs :
        ...
        self._w_fw = (
            frequency_fw.to_units("hr^-1").values.item() if frequency_fw is not None else np.pi
        )
        self._ad = (
    ...
    def input_properties(self):
        ...
        if self._apil:
            return_dict["air_pressure_on_interface_levels"] = {"dims": dims_stgz, "units": "Pa"}
        else:
