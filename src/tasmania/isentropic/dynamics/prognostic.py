...
class IsentropicPrognostic(AbstractFactory, DomainComponent, StencilFactory):
    ...
    def __init__(
        ...
        g = self.grid
        self._storage_shape = self.get_storage_shape(storage_shape, (g.nx, g.ny, g.nz + 1))

        # instantiate the class computing the numerical horizontal fluxes
        self._hflux = horizontal_flux_class.factory(horizontal_flux_scheme, backend=backend)
