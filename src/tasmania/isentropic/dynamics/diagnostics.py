...
class IsentropicDiagnostics(GridComponent, PhysicalConstantsComponent, StencilFactory):
    ...
    def __init__(
        ...
        *,
        ...
        self._theta[:nx, :ny, : nz + 1] = self.as_storage(
            data=grid.z_on_interface_levels.to_units("K").values[np.newaxis, np.newaxis, :]
        )
        ...
        self._stencil_diagnostic_variables = self.compile_stencil("diagnostic_variables")
        self._stencil_density_and_temperature = self.compile_stencil("density_and_temperature")
    ...
    def _diagnostic_variables_numpy(
        ...
        *,
        ...
        out_exn[i, j, kstart:kstop] = cp * (inout_p[i, j, kstart:kstop] / pref) ** (rd / cp)

        # compute the Montgomery potential
        mtg_s = in_theta[i, j, kstop - 1] * out_exn[i, j, kstop - 1] + g * in_hs[i, j, kstop - 1]
        ...
        for k in range(kstop - 2, kstart - 1, -1):
            inout_h[i, j, k] = inout_h[i, j, k + 1] - rd * (
                in_theta[i, j, k] * out_exn[i, j, k] + in_theta[i, j, k + 1] * out_exn[i, j, k + 1]
    ...
    def _height_numpy(
        ...
        *,
        ...
        for k in range(kstop - 2, kstart - 1, -1):
            inout_h[i, j, k] = inout_h[i, j, k + 1] - rd * (
                in_theta[i, j, k] * exn[i, j, k] + in_theta[i, j, k + 1] * exn[i, j, k + 1]
    ...
    def _density_and_temperature_numpy(
        ...
        *,
        ...
        out_t[i, j, k] = (
            0.5
            / cp
            * (in_theta[i, j, k] * in_exn[i, j, k] + in_theta[i, j, kp1] * in_exn[i, j, kp1])
        )
