...
def get_fourth_order_centered_flux_x_numpy(u, phi):
    flux = u[2:-2, :] / 12.0 * (7.0 * (phi[2:-2, :] + phi[1:-3, :]) - (phi[3:-1, :] + phi[:-4, :]))
    return flux
...
def get_fourth_order_centered_flux_y_numpy(v, phi):
    flux = v[:, 2:-2] / 12.0 * (7.0 * (phi[:, 2:-2] + phi[:, 1:-3]) - (phi[:, 3:-1] + phi[:, :-4]))
    return flux
...
def get_fourth_order_centered_flux_x_gt4py(u, phi):
    flux = (
        u[0, 0, 0] / 12.0 * (7.0 * (phi[0, 0, 0] + phi[-1, 0, 0]) - (phi[1, 0, 0] + phi[-2, 0, 0]))
    )
    return flux
...
def get_fourth_order_centered_flux_y_gt4py(v, phi):
    flux = (
        v[0, 0, 0] / 12.0 * (7.0 * (phi[0, 0, 0] + phi[0, -1, 0]) - (phi[0, 1, 0] + phi[0, -2, 0]))
    )
    return flux
