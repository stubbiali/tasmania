...
def step_forward_euler_momentum_numpy(
    ...
    *,
    ...
    su_new[i, j, k] = su_now[i, j, k] - dt * (
        ...
        + (1.0 - eps) * s_now[i, j, k] * (mtg_now[ip1, j, k] - mtg_now[im1, j, k]) / (2.0 * dx)
    ...
    sv_new[i, j, k] = sv_now[i, j, k] - dt * (
        ...
        + (1.0 - eps) * s_now[i, j, k] * (mtg_now[i, jp1, k] - mtg_now[i, jm1, k]) / (2.0 * dy)
...
@stencil_definition.register(backend="gt4py*", stencil="step_forward_euler_momentum")
def step_forward_euler_momentum_gt4py(
    ...
    *,
    ...
    from __externals__ import flux_dry, su_tnd_on, sv_tnd_on
    ...
    with computation(PARALLEL), interval(...):
        ...
        if __INLINED(su_tnd_on):  # compile-time if
            su_new = su_now[0, 0, 0] - dt * (
                ...
                + (1.0 - eps) * s_now[0, 0, 0] * (mtg_now[1, 0, 0] - mtg_now[-1, 0, 0]) / (2.0 * dx)
        ...
        if __INLINED(sv_tnd_on):  # compile-time if
            sv_new = sv_now[0, 0, 0] - dt * (
                ...
                + (1.0 - eps) * s_now[0, 0, 0] * (mtg_now[0, 1, 0] - mtg_now[0, -1, 0]) / (2.0 * dy)
                + eps * s_new[0, 0, 0] * (mtg_new[0, 1, 0] - mtg_new[0, -1, 0]) / (2.0 * dy)
                - sv_tnd[0, 0, 0]
            )
        else:
            sv_new = sv_now[0, 0, 0] - dt * (
