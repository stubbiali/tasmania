...
class RMSD(OfflineDiagnosticComponent):
    ...
    def input_properties(self):
        ...
        for name, units in self._fields.items():
            dimx = (
                g1.x_at_u_locations.dims[0] if "u_locations" in name else g1.x.dims[0]
            )
            dimy = (
                g1.y_at_v_locations.dims[0] if "v_locations" in name else g1.y.dims[0]
            )
            dimz = (
            ...
            dimx = (
                g2.x_at_u_locations.dims[0] if "u_locations" in name else g2.x.dims[0]
            )
            dimy = (
                g2.y_at_v_locations.dims[0] if "v_locations" in name else g2.y.dims[0]
            )
            dimz = (
