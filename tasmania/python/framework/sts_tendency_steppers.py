...
class RungeKutta2(STSTendencyStepper):
    ...
    def _call(self, state, prv_state, timestep):
        ...
        state_1 = multiply(
            ...
            add(
                state,
                add(prv_state, multiply(dt, k0), unshared_variables_in_output=False),
                units=out_units,
                unshared_variables_in_output=False,
            ),
        )
        state_1.update(
            {key: value for key, value in state.items() if key not in state_1}
        )
...
class RungeKutta3WS(STSTendencyStepper):
    ...
    def _call(self, state, prv_state, timestep):
        ...
        state_2 = multiply(
            ...
            add(
                state,
                add(prv_state, multiply(dt, k1), unshared_variables_in_output=False),
                units=out_units,
                unshared_variables_in_output=False,
            ),
        )
        state_2.update(
            {key: value for key, value in state.items() if key not in state_2}
        )
