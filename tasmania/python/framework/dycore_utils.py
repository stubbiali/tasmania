...
class StaticOperator:
    ...
    def get_input_properties(cls, dycore: "DynamicalCore") -> "PropertyDict":
        ...
        if dycore.fast_tendency_component is None:
            ...
            return_dict.update(
                sco1.get_properties_with_dims(dycore.fast_tendency_component)
            )
            sco2 = StaticComponentOperator.factory("diagnostic_properties")
            fast_diag_properties = sco2.get_properties_with_dims(
        ...
        if dycore.substeps > 0:
            ...
            superfast_input_properties = (
                ...
                else sco1.get_properties_with_dims(
            ...
            superfast_diag_properties = (
                {}
                if dycore.superfast_tendency_component is None
                else sco2.get_properties_with_dims(
                    dycore.superfast_tendency_component
                )
            )
            ...
            unshared_vars = tuple(
                name
                for name in superfast_input_properties
            ...
            unshared_vars = tuple(
                ...
                if not (
                    name in superfast_diag_properties or name in return_dict
                )
            )
            for name in unshared_vars:
                return_dict[name] = {}
    ...
    @classmethod
    def get_input_tendency_properties(
        cls, dycore: "DynamicalCore"
    ) -> "PropertyDict":
        return_dict = {}
        ...
        if dycore.fast_tendency_component is None:
            ...
            unshared_vars = tuple(
                name
                for name in stage_tendency_properties
                if name not in return_dict
            )
            for name in unshared_vars:
                return_dict[name] = {}
    ...
    def get_output_properties(cls, dycore: "DynamicalCore") -> "PropertyDict":
        ...
        if dycore.fast_diagnostic_component is not None:
            ...
            fast_diag_properties = sco4.get_properties_with_dims(
...
class StaticChecker:
    ...
    @classmethod
    def check_superfast_tendency_component(
        cls, dycore: "DynamicalCore"
    ) -> None:
        if dycore.substeps > 0:
            if (
                dycore.superfast_tendency_component is not None
                and not isinstance(
                    dycore.superfast_tendency_component,
    ...
    @classmethod
    def check_superfast_diagnostic_component(
        cls, dycore: "DynamicalCore"
    ) -> None:
        if dycore.substeps > 0:
            if (
                dycore.superfast_diagnostic_component is not None
                and not isinstance(
                    dycore.superfast_diagnostic_component,
...
class DynamicOperator:
    ...
    def get_ovewrite_tendencies(
        self, slow_tendencies: "DataArrayDict"
    ) -> Dict[str, bool]:
        out = {
            name: False
            for name in self.ftc_tp
            if name in self.fdc_tp or name in slow_tendencies
        }
        return out
