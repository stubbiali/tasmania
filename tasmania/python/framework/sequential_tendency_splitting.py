...
from tasmania.python.framework.sts_tendency_steppers import \
	STSTendencyStepper, tendencystepper_factory
from tasmania.python.utils.framework_utils import \
	check_property_compatibility, get_input_properties, get_output_properties
...
class SequentialTendencySplitting:
	...
	def _get_output_properties(self):
		...
		get_output_properties(
			sts_list, component_attribute_name="diagnostic_properties",
			return_dict=return_dict, consider_diagnostics=False
		)
		return return_dict
			...
			if not isinstance(component, self.__class__.allowed_diagnostic_type):
				...
				if substeps > 1:
					...
					for _ in range(1, substeps):
						_, state_aux = component(state_tmp, state_prv, timestep/substeps)
