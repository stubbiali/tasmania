...
class FifthOrderUpwind(NonconservativeFlux):
	...
	def get_flux_x(i, j, dx, u, h):
		...
		flux_x[i, j] = \
			u[i] / (60.0 * dx) * (
				45.0 * (h[i+1] - h[i-1]) - 9.0 * (h[i+2] - h[i-2]) +
				(h[i+3] - h[i-3])
			) - \
			(u[i] * (u[i] > 0.0) - u[i] * (u[i] < 0.0)) / (60.0 * dx) * (
				(h[i+3] + h[i-3]) - 6.0 * (h[i+2] + h[i-2]) +
				15.0 * (h[i+1] + h[i-1]) - 20.0 * h[i]
			)
		return flux_x
...
class IsentropicBoussinesqTendency(TendencyComponent):
	...
	def __init__(
		self, domain, advection_scheme='upwind', moist=False,
		...
		**kwargs:
		...
		self._stencil_velocity = gt.NGStencil(
			...
			inputs={
				'in_s': self._in_s, 'in_su': self._in_su, 'in_sv': self._in_sv
			},
		...
		inputs = {
			'in_b': self._tmp_b, 'in_s': self._in_s, 'in_ddmtg': self._in_ddmtg,
			'in_h': self._in_h, 'in_su': self._in_su, 'in_sv': self._in_sv
		}
		outputs = {
			'out_s': self._out_s, 'out_ddmtg': self._out_ddmtg,
			'out_su': self._out_su,	'out_sv': self._out_sv
		}
		if moist:
			inputs.update({
			...
			outputs.update({
				'out_qv': self._out_qv, 'out_qc': self._out_qc,
				'out_qr': self._out_qr
			})
	...
	def _stencil_tendencies_defs(
		in_b, in_s, in_ddmtg, in_h, in_su, in_sv,
