...
def stage_second_order(i, j, dx, dy, mtg, u_tnd, v_tnd):
    u_tnd[i, j] = (-mtg[i + 1, j] + mtg[i - 1, j]) / (2.0 * dx)
    v_tnd[i, j] = (-mtg[i, j + 1] + mtg[i, j - 1]) / (2.0 * dy)


def stage_fourth_order(i, j, dx, dy, mtg, u_tnd, v_tnd):
    u_tnd[i, j] = (
        -mtg[i - 2, j] + 8.0 * mtg[i - 1, j] - 8.0 * mtg[i + 1, j] + mtg[i + 2, j]
    ) / (12.0 * dx)
    v_tnd[i, j] = (
        -mtg[i, j - 2] + 8.0 * mtg[i, j - 1] - 8.0 * mtg[i, j + 1] + mtg[i, j + 2]
    ) / (12.0 * dy)
...
def stage_weighted(i, j, k, dx, dy, eps, mtg, p, u_tnd, v_tnd):
    ...
    pgx_avg[i, j] = (
        wgtx_w[i, j] * pgx_w[i, j]
        + wgtx_e[i, j] * pgx_e[i, j]
        + wgtx_s[i, j] * pgx_s[i, j]
        + wgtx_n[i, j] * pgx_n[i, j]
    ...
    pgy_avg[i, j] = (
        wgty_w[i, j] * pgy_w[i, j]
        + wgty_e[i, j] * pgy_e[i, j]
        + wgty_s[i, j] * pgy_s[i, j]
        + wgty_n[i, j] * pgy_n[i, j]
    ) / (wgty_w[i, j] + wgty_e[i, j] + wgty_s[i, j] + wgty_n[i, j])

    u_tnd[i, j] = ((wgtx_c[i, j] - 1.0) * pgx_avg[i, j] - wgtx_c[i, j] * pgx_c[i, j]) / (
        2.0 * dx
    )
    v_tnd[i, j] = ((wgty_c[i, j] - 1.0) * pgy_avg[i, j] - wgty_c[i, j] * pgy_c[i, j]) / (
        2.0 * dy
    )
...
class IsentropicNonconservativePressureGradient(TendencyComponent):
    ...
    def __init__(self, domain, scheme, backend=gt.mode.NUMPY, dtype=datatype, **kwargs):
        """
		Parameters
		----------
