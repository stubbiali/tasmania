...
class ForwardEulerSI(IsentropicPrognostic):
    ...
    def stage_call(self, stage, timestep, state, tendencies=None):
        ...
        if self._moist:
            out_state.update(
                {
                    "isentropic_density_of_water_vapor": self._sqv_new.data[
                        :nx, :ny, :nz
                    ],
                    "isentropic_density_of_cloud_liquid_water": self._sqc_new.data[
                        :nx, :ny, :nz
                    ],
                    "isentropic_density_of_precipitation_water": self._sqr_new.data[
                        :nx, :ny, :nz
                    ],
                }
            )
...
class RK3WSSI(IsentropicPrognostic):
    ...
    def _stencils_set_inputs(self, stage, state, tendencies):
        ...
        if self._moist:
            self._sqv_int.data[:nx, :ny, :nz] = state[
                "isentropic_density_of_water_vapor"
            ][...]
            self._sqc_int.data[:nx, :ny, :nz] = state[
                "isentropic_density_of_cloud_liquid_water"
            ][...]
            self._sqr_int.data[:nx, :ny, :nz] = state[
