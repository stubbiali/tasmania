...
class Centered(IsentropicBoussinesqMinimalPrognostic):
	...
	def stage_call(self, stage, timestep, state, tendencies=None):
		...
		self._in_ddmtg_old[...] = self._in_ddmtg[...]
		if self._moist:
	...
	def _stage_stencil_initialize(self, tendencies):
		...
		_inputs = {
			...
			'in_s': self._in_s, 'in_s_old': self._in_s_old,
			'in_su': self._in_su, 'in_su_old': self._in_su_old,
			'in_sv': self._in_sv, 'in_sv_old': self._in_sv_old,
			'in_ddmtg': self._in_ddmtg, 'in_ddmtg_old': self._in_ddmtg_old,
		}
		_outputs = {
	...
	def _stage_stencil_allocate_inputs(self, tendencies):
		...
		self._in_ddmtg_old = np.zeros((nx, ny, nz), dtype=dtype)
		if self._moist:
	...
	def _stage_stencil_set_inputs(self, stage, timestep, state, tendencies):
		...
		if self._is_first_timestep:
			...
			self._in_ddmtg_old[...] = state['dd_montgomery_potential']
			if self._moist:
	...
	def _stage_stencil_defs(
		self, dt, in_s, in_u, in_v, in_su, in_sv, in_ddmtg,
		in_s_old, in_su_old, in_sv_old, in_ddmtg_old,
		...
		fluxes = self._hflux(
			i, j, dt, in_s, in_u, in_v, in_su, in_sv, in_ddmtg,
		...
		flux_ddmtg_x, flux_ddmtg_y = fluxes[6], fluxes[7]
		if self._moist:
			flux_sqv_x, flux_sqv_y = fluxes[8], fluxes[9]
			flux_sqc_x, flux_sqc_y = fluxes[10], fluxes[11]
			flux_sqr_x, flux_sqr_y = fluxes[12], fluxes[13]
		...
		# advance the second derivative of the Montgomery potential
		out_ddmtg[i, j] = in_ddmtg_old[i, j] - 2. * dt * (
			(flux_ddmtg_x[i, j] - flux_ddmtg_x[i-1, j]) / dx +
			(flux_ddmtg_y[i, j] - flux_ddmtg_y[i, j-1]) / dy
		)
		...
		if not self._moist:
			return out_s, out_su, out_sv, out_ddmtg
		else:
			return out_s, out_su, out_sv, out_ddmtg, out_sqv, out_sqc, out_sqr
...
class ForwardEuler(IsentropicBoussinesqMinimalPrognostic):
	...
	def _stage_stencil_initialize(self, tendencies):
		...
		_inputs = {
			...
			'in_su': self._in_su, 'in_sv': self._in_sv, 'in_ddmtg': self._in_ddmtg
		}
		_outputs = {
			'out_s': self._out_s, 'out_su': self._out_su, 'out_sv': self._out_sv,
			'out_ddmtg': self._out_ddmtg
	...
	def _stage_stencil_defs(
		...
		# advance the second derivative of the Montgomery potential
		out_ddmtg[i, j] = in_ddmtg[i, j] - dt * (
			((flux_ddmtg_x[i, j] - flux_ddmtg_x[i-1, j]) / dx if md != 'y' else 0) +
			((flux_ddmtg_y[i, j] - flux_ddmtg_y[i, j-1]) / dy if md != 'x' else 0)
		)
...
class RK2(IsentropicBoussinesqMinimalPrognostic):
	...
	def _stage_stencil_initialize(self, tendencies):
		...
		_inputs = {
			...
			'in_ddmtg': self._in_ddmtg, 'in_ddmtg_int': self._in_ddmtg_int
		}
		_outputs = {
			'out_s': self._out_s, 'out_su': self._out_su, 'out_sv': self._out_sv,
			'out_ddmtg': self._out_ddmtg
	...
	def _stage_stencil_allocate_inputs(self, tendencies):
		...
		self._in_ddmtg_int = np.zeros((nx, ny, nz), dtype=dtype)
		if self._moist:
	...
	def _stage_stencil_set_inputs(self, stage, timestep, state, tendencies):
		...
		self._in_ddmtg_int[...] = state['dd_montgomery_potential'][...]
		if self._moist:
	...
	def _stage_stencil_defs(
		...
		in_su, in_su_int, in_sv, in_sv_int, in_ddmtg, in_ddmtg_int,
		...
		fluxes = self._hflux(
			...
			in_ddmtg_int, in_sqv_int, in_sqc_int, in_sqr_int,
		)
		flux_s_x, flux_s_y = fluxes[0], fluxes[1]
		flux_su_x, flux_su_y = fluxes[2], fluxes[3]
		flux_sv_x, flux_sv_y = fluxes[4], fluxes[5]
...
class RK3(IsentropicBoussinesqMinimalPrognostic):
	...
	def _stage_stencils_initialize(self, tendencies):
		...
		_outputs = {
			...
			'out_ddmtg0': self._ddmtg0, 'out_ddmtg': self._out_ddmtg
		}
		if self._moist:
			_inputs.update({
		...
		_inputs = {
			'in_s': self._in_s, 'in_s_int': self._in_s_int,
			'in_s0': self._s0, 'in_s1': self._s1,
			'in_u_int': self._in_u, 'in_v_int': self._in_v,
			'in_su': self._in_su, 'in_su_int': self._in_su_int,
			'in_su0': self._su0, 'in_su1': self._su1,
			'in_sv': self._in_sv, 'in_sv_int': self._in_sv_int,
			'in_sv0': self._sv0, 'in_sv1': self._sv1,
			'in_ddmtg': self._in_ddmtg, 'in_ddmtg_int': self._in_ddmtg_int,
			'in_ddmtg0': self._ddmtg0, 'in_ddmtg1': self._ddmtg1
		}
		_outputs = {
			'out_s': self._out_s, 'out_su': self._out_su, 'out_sv': self._out_sv,
			'out_ddmtg': self._out_ddmtg
		...
		if self._moist:
			_inputs.update({
				'in_sqv': self._in_sqv, 'in_sqv_int': self._in_sqv_int,
				'in_sqv0': self._sqv0, 'in_sqv1': self._sqv1,
				'in_sqc': self._in_sqc, 'in_sqc_int': self._in_sqc_int,
				'in_sqc0': self._sqc0, 'in_sqc1': self._sqc1,
				'in_sqr': self._in_sqr, 'in_sqr_int': self._in_sqr_int,
				'in_sqr0': self._sqr0, 'in_sqr1': self._sqr1,
			})
			_outputs.update({
				'out_sqv': self._out_sqv, 'out_sqc': self._out_sqc,
				'out_sqr': self._out_sqr,
	...
	def _stage_stencils_allocate_temporaries(self):
		...
		self._ddmtg0 = np.zeros((nx, ny, nz), dtype=dtype)
		if self._moist:
		...
		self._ddmtg1 = np.zeros((nx, ny, nz), dtype=dtype)
		if self._moist:
	...
	def _stage_stencil_first_defs(
		...
		out_ddmtg[i, j] = in_ddmtg[i, j] + a1 * out_ddmtg0[i, j]
	...
	def _stage_stencil_second_defs(
		...
		# instantiate fields representing the second increments
		out_s1 = gt.Equation()
		out_su1 = gt.Equation()
		out_sv1 = gt.Equation()
		out_ddmtg1 = gt.Equation()
		if self._moist:
		...
		out_ddmtg[i, j] = in_ddmtg[i, j] + \
			b21 * in_ddmtg0[i, j] + (a2 - b21) * out_ddmtg1[i, j]
		...
		if self._moist:
			...
			out_sqv[i, j] = in_sqv[i, j] + \
				b21 * in_sqv0[i, j] + (a2 - b21) * out_sqv1[i, j]
			...
			out_sqc[i, j] = in_sqc[i, j] + \
				b21 * in_sqc0[i, j] + (a2 - b21) * out_sqc1[i, j]
			...
			out_sqr[i, j] = in_sqr[i, j] + \
				b21 * in_sqr0[i, j] + (a2 - b21) * out_sqr1[i, j]

		if not self._moist:
			return out_s1, out_s, out_su1, out_su, out_sv1, out_sv, \
				out_ddmtg1, out_ddmtg
		else:
			return out_s1, out_s, out_su1, out_su, out_sv1, out_sv, \
				out_ddmtg1, out_ddmtg, out_sqv1, out_sqv, out_sqc1, out_sqc, \
				out_sqr1, out_sqr
	...
	def _stage_stencil_third_defs(
		...
		in_ddmtg, in_ddmtg_int, in_ddmtg0, in_ddmtg1,
		...
		# instantiate fields representing the third increments
		tmp_s2 = gt.Equation()
		tmp_su2 = gt.Equation()
		tmp_sv2 = gt.Equation()
		tmp_ddmtg2 = gt.Equation()
		if self._moist:
		...
		out_ddmtg[i, j] = in_ddmtg[i, j] + \
			g0 * in_ddmtg0[i, j] + g1 * in_ddmtg1[i, j] + g2 * tmp_ddmtg2[i, j]
