...
def get_centered_flux_x_numpy(u: np.ndarray, phi: np.ndarray) -> np.ndarray:
    flux = u[1:-1, :] * 0.5 * (phi[:-2, :] + phi[1:-1, :])
    return flux


def get_centered_flux_y_numpy(v: np.ndarray, phi: np.ndarray) -> np.ndarray:
...
def get_fourth_order_centered_flux_x_numpy(u: np.ndarray, phi: np.ndarray) -> np.ndarray:
    flux = (
        u[2:-2, :]
        / 12.0
        * (7.0 * (phi[2:-2, :] + phi[1:-3, :]) - (phi[3:-1, :] + phi[:-4, :]))
    )
    return flux


def get_third_order_upwind_flux_x_numpy(u: np.ndarray, phi: np.ndarray) -> np.ndarray:
...
def get_fourth_order_centered_flux_y_numpy(v: np.ndarray, phi: np.ndarray) -> np.ndarray:
    flux = (
        v[:, 2:-2]
        / 12.0
        * (7.0 * (phi[:, 2:-2] + phi[:, 1:-3]) - (phi[:, 3:-1] + phi[:, :-4]))
...
def get_third_order_upwind_flux_y_numpy(v: np.ndarray, phi: np.ndarray) -> np.ndarray:
...
def get_sixth_order_centered_flux_x_numpy(u: np.ndarray, phi: np.ndarray) -> np.ndarray:
    flux = (
        u[3:-3, :]
        / 60.0
        * (
...
def get_fifth_order_upwind_flux_x_numpy(u: np.ndarray, phi: np.ndarray) -> np.ndarray:
...
def get_sixth_order_centered_flux_y_numpy(v: np.ndarray, phi: np.ndarray) -> np.ndarray:
    flux = (
        v[:, 3:-3]
        / 60.0
        * (
...
def get_fifth_order_upwind_flux_y_numpy(v: np.ndarray, phi: np.ndarray) -> np.ndarray:
