...
class IsentropicDiagnostics:
    ...
    def __init__(
        ...
        *,
        ...
        # auxiliary fields
        theta_1d = grid.z_on_interface_levels.to_units("K").values[
        ...
        descriptor = get_storage_descriptor(
            shape, dtype, halo=halo, mask=[True, True, True]
        )
        self._theta = gt.storage.from_array(
            theta, descriptor=descriptor, backend=backend
        )
    ...
    def get_diagnostic_variables(self, s, pt, p, exn, mtg, h):
        ...
        self._topo.data[:-1, :-1, -1] = self._grid.topography.profile.to_units(
            "m"
        ).values[...]
