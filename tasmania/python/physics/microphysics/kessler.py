...
class KesslerMicrophysics(TendencyComponent):
	...
	def __init__(
		...
		saturation_water_vapor_formula='tetens', physical_constants=None, *,
		...
		**kwargs :
		...
		# allocate the gt4py storage collecting inputs and outputs
		...
		self._in_p = gt.storage.zeros(descriptor, backend=backend)
		self._in_ps = gt.storage.zeros(descriptor, backend=backend)
		self._in_exn = gt.storage.zeros(descriptor, backend=backend)
		self._in_qc = gt.storage.zeros(descriptor, backend=backend)
		self._in_qr = gt.storage.zeros(descriptor, backend=backend)
		self._out_qc_tnd = gt.storage.zeros(descriptor, backend=backend)
		self._out_qr_tnd = gt.storage.zeros(descriptor, backend=backend)
		if rain_evaporation:
			self._in_qv = gt.storage.zeros(descriptor, backend=backend)
			self._out_qv_tnd = gt.storage.zeros(descriptor, backend=backend)
			self._out_theta_tnd = gt.storage.zeros(descriptor, backend=backend)

		# initialize the underlying gt4py stencil object
		decorator = gt.stencil(
			backend, backend_opts=backend_opts, build_info=build_info,
			min_signature=True, rebuild=rebuild, module='kessler_microphysics',
			externals={
				'air_pressure_on_interface_levels': air_pressure_on_interface_levels,
				'beta': beta, 'lhvw': pcs['latent_heat_of_vaporization_of_water'],
	...
	def array_call(self, state):
		...
		if self._air_pressure_on_interface_levels:
			self._in_p.data[...] = state['air_pressure_on_interface_levels']
			self._in_exn.data[...] = state['exner_function_on_interface_levels']
		else:
			self._in_p.data[:, :, :nz] = state['air_pressure']
			self._in_exn.data[:, :, :nz] = state['exner_function']

		# compute the saturation water vapor pressure
		self._in_ps.data[:, :, :nz] = self._swvf(state['air_temperature'])

		# collect the stencil arguments
		stencil_args = {
			'a': self._a, 'k1': self._k1, 'k2': self._k2,
			'in_rho': self._in_rho, 'in_p': self._in_p, 'in_ps': self._in_ps,
			'in_exn': self._in_exn, 'in_qc': self._in_qc, 'in_qr': self._in_qr,
			'out_qc_tnd': self._out_qc_tnd, 'out_qr_tnd': self._out_qr_tnd
		}
		if self._rain_evaporation:
			stencil_args['in_qv'] = self._in_qv
			stencil_args['out_qv_tnd'] = self._out_qv_tnd
			stencil_args['out_theta_tnd'] = self._out_theta_tnd

		# run the stencil
		self._stencil(
			**stencil_args, origin={'_all_': (0, 0, 0)}, domain=(nx, ny, nz)
		)

		# collect the tendencies
		# >>> comment the following two lines before testing <<<
		self._out_qc_tnd.data[np.isnan(self._out_qc_tnd.data)] = 0.0
		self._out_qr_tnd.data[np.isnan(self._out_qr_tnd.data)] = 0.0
		tendencies = {
			mfcw: self._out_qc_tnd.data[:, :, :nz],
			mfpw: self._out_qr_tnd.data[:, :, :nz],
		}
		if self._rain_evaporation:
			# >>> comment the following two lines before testing <<<
			self._out_qv_tnd.data[np.isnan(self._out_qv_tnd.data)] = 0.0
			self._out_theta_tnd.data[np.isnan(self._out_theta_tnd.data)] = 0.0
			tendencies[mfwv] = self._out_qv_tnd.data[:, :, :nz]
			if not self._pttd:
				tendencies['air_potential_temperature'] = \
					self._out_theta_tnd.data[:, :, :nz]
	...
	def _stencil_defs(
		...
		*,
		...
		if rain_evaporation:
			...
			er = (1. - in_qv[0, 0, 0] / qvs[0, 0, 0]) * c[0, 0, 0] * \
				((rho_gcm3[0, 0, 0] * in_qr[0, 0, 0]) ** 0.525) / \
				(rho_gcm3[0, 0, 0] * (5.4e5 + 2.55e6 / (p_mbar[0, 0, 0] * qvs[0, 0, 0])))
...
class KesslerSaturationAdjustment(DiagnosticComponent):
	...
	def __init__(
		self, domain, grid_type='numerical',
		air_pressure_on_interface_levels=True, physical_constants=None, *,
		backend='numpy', backend_opts=None, build_info=None, dtype=datatype,
		...
		# allocate the gt4py storages collecting the stencil inputs and outputs
		nx, ny, nz = self.grid.nx, self.grid.ny, self.grid.nz
		descriptor = get_storage_descriptor((nx, ny, nz+1), dtype, halo=halo)
		self._in_p = gt.storage.zeros(descriptor, backend=backend)
		self._in_ps = gt.storage.zeros(descriptor, backend=backend)
		self._in_t = gt.storage.zeros(descriptor, backend=backend)
		self._in_qv = gt.storage.zeros(descriptor, backend=backend)
		self._in_qc = gt.storage.zeros(descriptor, backend=backend)
		...
		decorator = gt.stencil(
			backend, backend_opts=backend_opts, build_info=build_info,
			rebuild=rebuild, module='kessler_saturation_adjustment',
			externals={
				'air_pressure_on_interface_levels': air_pressure_on_interface_levels,
				'beta': beta, 'lhvw': lhvw, 'cp': cp
			}
		)
		self._stencil = decorator(self._stencil_defs)
	...
	def array_call(self, state):
		...
		# extract the required model variables
		self._in_t.data[:, :, :nz] = state['air_temperature']
		self._in_qv.data[:, :, :nz] = state['mass_fraction_of_water_vapor_in_air']
		self._in_qc.data[:, :, :nz] = state['mass_fraction_of_cloud_liquid_water_in_air']
		if self._apoil:
			self._in_p.data[...] = state['air_pressure_on_interface_levels']
		else:
			self._in_p.data[:, :, :nz] = state['air_pressure']
		...
		# compute the saturation water vapor pressure
		self._in_ps.data[:, :, :nz] = tetens_formula(state['air_temperature'])

		# run the stencil
		self._stencil(
			in_p=self._in_p, in_ps=self._in_ps, in_t=self._in_t, in_qv=self._in_qv,
			in_qc=self._in_qc, out_qv=self._out_qv, out_qc=self._out_qc,
		...
		# collect the diagnostics
		diagnostics = {
			mfwv: self._out_qv.data[:, :, :nz],
			mfcw: self._out_qc.data[:, :, :nz],
		}
...
class KesslerFallVelocity(DiagnosticComponent):
	...
	def __init__(
		...
		descriptor2d = get_storage_descriptor(
			(nx, ny, nz), dtype, halo=halo, mask=[True, True, True]
		)
		self._in_rho = gt.storage.zeros(descriptor, backend=backend)
		self._in_rho_s = gt.storage.zeros(descriptor2d, backend=backend)
		self._in_qr  = gt.storage.zeros(descriptor, backend=backend)
		self._out_vt = gt.storage.zeros(descriptor, backend=backend)

		decorator = gt.stencil(
			backend, backend_opts=backend_opts, build_info=build_info,
			rebuild=rebuild, module='kessler_fall_velocity'
		)
		self._stencil = decorator(self._stencil_defs)
	...
	def array_call(self, state):
		...
		self._in_rho_s.data[...] = self._in_rho.data[:, :, -1:]

		# run the stencil
		self._stencil(
			in_rho=self._in_rho, in_rho_s=self._in_rho_s,
			in_qr=self._in_qr, out_vt=self._out_vt,
			origin={'_all_': (0, 0, 0)}, domain=(nx, ny, nz),
			exec_info=self._exec_info
		)

		# collect the diagnostics
		diagnostics = {'raindrop_fall_velocity': self._out_vt.data}
	...
	def _stencil_defs(
		...
		out_vt = 36.34 * \
			(1.e-3 * in_rho[0, 0, 0] * (in_qr[0, 0, 0] > 0.) * in_qr[0, 0, 0]) ** 0.1346 * \
...
class KesslerSedimentation(ImplicitTendencyComponent):
	...
	def __init__(
		...
		**kwargs :
		...
		decorator = gt.stencil(
			backend, backend_opts=backend_opts, build_info=build_info,
			rebuild=rebuild, module='kessler_sedimentation',
	...
	def array_call(self, state, timestep):
		...
		self._in_qr.data[:, :, :nz] = state[mfpw]
		self._in_vt.data[:, :, :nz] = state['raindrop_fall_velocity']

		self._stencil(
			in_rho=self._in_rho, in_h=self._in_h, in_qr=self._in_qr,
			in_vt=self._in_vt, out_qr=self._out_qr, dt=dt,
