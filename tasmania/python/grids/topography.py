...
class Topography1d:
	...
	"""
	def __init__(
		self, x, topo_type='flat_terrain', topo_time=timedelta(),
		...
		if self.topo_type == 'flat_terrain':
			...
			topo_width_x_ = kwargs.get(
				'topo_width_x', DataArray(1.0, attrs={'units': x.attrs['units']})
			)
			...
			try:
				from tasmania.cpp.parser.parser_1d import Parser1d
			except ImportError:
		...
		if self.topo_kwargs['topo_smooth']:
			self._topo_final[1:-1] += 0.25 * (
				self._topo_final[:-2] - 2.*self._topo_final[1:-1] + self._topo_final[2:]
			)

		self.topo = DataArray(
			self.topo_fact*self._topo_final,
...
class Topography2d:
	...
	*Journal of Atmospheric Sciences*, *54*:534-554.
	...
	"""
	def __init__(
		self, grid, topo_type='flat_terrain', topo_time=timedelta(),
		dtype=datatype, **kwargs
	):
		"""
		Constructor.
		...
		if self.topo_type == 'flat_terrain':
			...
			topo_width_y_ = kwargs.get(
				'topo_width_y', DataArray(1.0, attrs={'units': y.attrs['units']})
			)
