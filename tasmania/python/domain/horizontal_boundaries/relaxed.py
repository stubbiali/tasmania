...
class Relaxed(HorizontalBoundary):
    ...
    def enforce_field(
        ...
        if not self._gt_powered:
            ...
            field[i, j, k] -= g[i, j, k] * (field[i, j, k] - field_ref[i, j, k])
    ...
    def _allocate_coefficient_matrix(self, field_shape=None):
        ...
        if not self._gt_powered:
            ...
            self._gamma = zeros(
                (nx + 1, ny + 1, 1), gt_powered=False, backend=backend, dtype=dtype
            )
        else:
...
class Relaxed1DX(HorizontalBoundary):
    ...
    def _allocate_coefficient_matrix(self, field_shape=None):
        ...
        if not self._gt_powered:
            # create a single coefficient matrix
            self._gamma = zeros(
                (nx + 1, 2 * nb + 2, 1), gt_powered=False, backend=backend, dtype=dtype
            )
        else:
            # get the proper storage shape
...
class Relaxed1DY(HorizontalBoundary):
    ...
    def _allocate_coefficient_matrix(self, field_shape=None):
        ...
        if not self._gt_powered:
            # create a single coefficient matrix
            self._gamma = zeros(
                (2 * nb + 2, ny + 1, 1), gt_powered=False, backend=backend, dtype=dtype
            )
        else:
            # get the proper storage shape
