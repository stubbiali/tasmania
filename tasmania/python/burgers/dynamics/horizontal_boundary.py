...
class ZhaoHorizontalBoundary(HorizontalBoundary):
	...
	def from_computational_to_physical_domain(self, phi_, out_dims=None, change_sign=None):
		return phi_
	...
	def enforce(self, phi_new, phi_now, field_name, time):
		...
		phi_new[-nb:, :, :] = self._solution_factory(
			g, t, slice_x=slice(g.nx-nb, g.nx), slice_y=None, field_name=field_name
		)
		phi_new[:, :nb, :] = self._solution_factory(
			g, t, slice_x=None, slice_y=slice(0, nb), field_name=field_name
		)
		phi_new[:, -nb:, :] = self._solution_factory(
			g, t, slice_x=None, slice_y=slice(g.ny-nb, g.ny), field_name=field_name
		)
