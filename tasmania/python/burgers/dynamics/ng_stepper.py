...
class NGBurgersStepper(abc.ABC):
    """
...
class ForwardEuler(NGBurgersStepper):
    """
    ...
    def __call__(self, stage, state, tendencies, timestep):
        ...
        stencil_args = {
            ...
            "in_u_tmp": state["x_velocity"],
            "in_v_tmp": state["y_velocity"],
            "out_u": self._out_u,
            "out_v": self._out_v,
        }
        if "x_velocity" in tendencies:
            stencil_args["in_u_tnd"] = tendencies["x_velocity"]
            self._in_u_tnd = tendencies["x_velocity"]
        elif self._in_u_tnd is not None:
            stencil_args["in_u_tnd"] = tendencies["x_velocity"]
        if "y_velocity" in tendencies:
            stencil_args["in_v_tnd"] = tendencies["y_velocity"]
            self._in_v_tnd = tendencies["y_velocity"]
        elif self._in_v_tnd is not None:
            stencil_args["in_v_tnd"] = tendencies["y_velocity"]
    ...
    def _stencil_initialize(self, tendencies):
        ...
        self._out_u = gt.storage.zeros(descriptor, backend=backend)
        self._out_v = gt.storage.zeros(descriptor, backend=backend)
