    ...
    for key in set().union(state_1.keys(), state_2.keys()):
        if key != "time":
            if (state_1.get(key, None) is not None) and (
                ...
                out_da = (
                    deepcopy(state_2[key])
                    if units_are_same(state_2[key].attrs["units"], _units)
                    else state_2[key].to_units(_units)
                )
    ...
    for key in set().union(state_1.keys(), state_2.keys()):
        if key != "time":
            if (state_1.get(key, None) is not None) and (
                ...
                _units = units.get(key, state_2[key].attrs["units"])
                out_da = (
                    deepcopy(state_1[key])
                    if units_are_same(state_1[key].attrs["units"], _units)
                    else state_1[key].to_units(_units)
                )
