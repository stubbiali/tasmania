...
class Quiver(Drawer):
	...
	"""
	def __init__(
		self, grid, x=None, y=None, z=None,
...
def make_quiver_xy(
	grid, xaxis_units, yaxis_units, xcomp_retriever, ycomp_retriever, scalar_retriever,
	state, fig, ax, **kwargs
):
...
def make_quiver_xz(
	grid, xaxis_units, zaxis_units, xcomp_retriever, zcomp_retriever, scalar_retriever,
	state, fig, ax, **kwargs
):
	vx = np.squeeze(xcomp_retriever(state))
...
def make_quiver_xh(
	grid, xaxis_units, zaxis_retriever, xcomp_retriever, zcomp_retriever, scalar_retriever,
	state, fig, ax, **kwargs
):
...
def make_quiver_yz(
	grid, yaxis_units, zaxis_units, ycomp_retriever, zcomp_retriever, scalar_retriever,
	state, fig, ax, **kwargs
):
...
def make_quiver_yh(
	grid, yaxis_units, zaxis_retriever, xcomp_retriever, zcomp_retriever, scalar_retriever,
	state, fig, ax, **kwargs
):
	raise NotImplementedError()
