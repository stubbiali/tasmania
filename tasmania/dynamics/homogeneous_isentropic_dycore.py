...
This module contains:
	...
	SUSHomogeneousIsentropicDynamicalCore(SequentialSplittingDynamicalCore)
	SSUSHomogeneousIsentropicDynamicalCore(SequentialSplittingDynamicalCore)
"""
import numpy as np
...
class HomogeneousIsentropicDynamicalCore(DynamicalCore):
	...
	def __init__(self, grid, moist_on, time_integration_scheme,
		...
		self._prognostic = HomogeneousIsentropicPrognostic.factory(
			time_integration_scheme, 'xy', grid, moist_on, self._boundary,
...
class SUSHomogeneousIsentropicDynamicalCore(SequentialSplittingDynamicalCore):
	...
	def __init__(self, grid, moist_on, time_integration_scheme,
		...
		self._prognostic_x = HomogeneousIsentropicPrognostic.factory(
			time_integration_scheme, 'x', grid, moist_on, self._boundary,
			horizontal_flux_scheme, backend, dtype)
		self._prognostic_y = HomogeneousIsentropicPrognostic.factory(
			time_integration_scheme, 'y', grid, moist_on, self._boundary,
			horizontal_flux_scheme, backend, dtype)
	...
	def _prognostic_call(self, raw_state, timestep):
		...
		for k in range(ns):
			...
			self._boundary.set_outermost_layers_x(u_new, raw_state['x_velocity_at_u_locations'])
			self._boundary.set_outermost_layers_y(v_new, raw_state['y_velocity_at_v_locations'])
...
class SSUSHomogeneousIsentropicDynamicalCore(SUSHomogeneousIsentropicDynamicalCore):
	"""
	This class inherits
	...
	def __init__(self, grid, moist_on, time_integration_scheme,
		...
		"""
		super().__init__(
			grid, moist_on, time_integration_scheme,
			horizontal_flux_scheme, horizontal_boundary_type,
			damp_on, damp_type, damp_depth, damp_max,
			smooth_on, smooth_type, smooth_damp_depth,
			smooth_coeff, smooth_coeff_max,
			smooth_moist_on, smooth_moist_type,	smooth_moist_damp_depth,
			smooth_moist_coeff,	smooth_moist_coeff_max,
			backend, dtype)
	...
	def _prognostic_call(self, raw_state, timestep):
		...
		# Advance the solution considering only the y-advection
		for k in range(ns):
			raw_state_tmp = self._prognostic_y(k, 0.5*timestep, raw_state_new)
			raw_state_new.update(raw_state_tmp)
