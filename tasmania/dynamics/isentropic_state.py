...
def get_default_isentropic_state(grid, time, x_velocity, y_velocity, brunt_vaisala,
					  			 moist_on=False, dtype=datatype, physical_constants=None):
	# Shortcuts
	...
	if moist_on:
		...
		from tasmania.utils.meteo_utils import convert_relative_humidity_to_water_vapor
		qv = convert_relative_humidity_to_water_vapor('goff_gratch', p_unstg_,
													  state['air_temperature'], rh_)
