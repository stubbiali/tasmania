...
class Upwind(VerticalHomogeneousIsentropicFlux):
	...
	def __call__(self, k, w, s, su, sv, qv=None, qc=None, qr=None):
		...
		if self._moist_on:
			out_qv = _get_upwind_flux(k, w, qv)
			out_qc = _get_upwind_flux(k, w, qc)
			out_qr = _get_upwind_flux(k, w, qr)
...
class Centered(VerticalHomogeneousIsentropicFlux):
	...
	def __call__(self, k, w, s, su, sv, qv=None, qc=None, qr=None):
		...
		if self._moist_on:
			out_qv = _get_centered_flux(k, w, qv)
			out_qc = _get_centered_flux(k, w, qc)
			out_qr = _get_centered_flux(k, w, qr)
...
class ThirdOrderUpwind(VerticalHomogeneousIsentropicFlux):
	...
	def __call__(self, k, w, s, su, sv, qv=None, qc=None, qr=None):
		...
		if self._moist_on:
			out_qv = _get_third_order_upwind_flux(k, w, qv)
			out_qc = _get_third_order_upwind_flux(k, w, qc)
			out_qr = _get_third_order_upwind_flux(k, w, qr)
...
class FifthOrderUpwind(VerticalHomogeneousIsentropicFlux):
	...
	def __call__(self, k, w, s, su, sv, qv=None, qc=None, qr=None):
		...
		if self._moist_on:
			out_qv = _get_fifth_order_upwind_flux(k, w, qv)
			out_qc = _get_fifth_order_upwind_flux(k, w, qc)
			out_qr = _get_fifth_order_upwind_flux(k, w, qr)
