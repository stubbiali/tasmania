...
class _Centered(IsentropicPrognostic):
	...
	def _stencil_stepping_by_neglecting_vertical_motion_initialize(
		...
		if raw_tendencies is not None:
			if raw_tendencies.get('tendency_of_mass_fraction_of_water_vapor_in_air', None) \
				is not None:
				_inputs['in_qv_tnd'] = self._in_qv_tnd
			if raw_tendencies.get('tendency_of_mass_fraction_of_cloud_liquid_water_in_air', None) \
				is not None:
				_inputs['in_qc_tnd'] = self._in_qc_tnd
			if raw_tendencies.get('tendency_of_mass_fraction_of_precipitation_water_in_air', None) \
				is not None:
