...
class SLEVE2d(XZGrid):
	...
	The vertical coordinate :math:`\mu` may be formulated to define a hybrid terrain-
	following coordinate system with terrain-following coordinate lines between the 
	surface terrain-height and :math:`\mu = \mu_F`, where :math:`\mu`-coordinate 
	...
	Attributes:
		...
		z (obj): :class:`~grids.axis.Axis` object storing the :math:`\mu`-main levels.
		z_half_levels (obj): :class:`~grids.axis.Axis` object storing the :math:`\mu`-half levels.
		nz (int): Number of vertical main levels.
		dz (float): The :math:`\mu`-spacing.
		z_interface (float): The interface coordinate :math:`\mu_F`.
		...
		height_half_levels (obj): :class:`xarray.DataArray` storing the geometrical height at
			the half levels.
		height_interface (float): geometrical height corresponding to :math:`\mu = \mu_F`.
	...
	def __init__(self, domain_x, nx, domain_z, nz, 
		...
		Args:
			...
			domain_z (tuple): Tuple in the form :math:`(\mu_{top}, ~ \mu_{surface})`.
			nz (int): Number of vertical main levels.
			...
			z_interface (`float`, optional): Interface value :math:`\mu_F`. If not specified,
				it is assumed that :math:`\mu_F = \mu_T`, with :math:`\mu_T` the value of 
				:math:`\mu` at the top of the domain. In other words, a fully terrain-following 
				coordinate system is supposed.
			N (`int`, optional): Number of filter iterations performed to determine the
				large-scale component of the surface terrain-height. Defaults to 100.
			s1 (`float`, optional): Large-scale decay constant. Defaults to :math:`8000 ~ m`.
			s2 (`float`, optional): Small-scale decay constant. Defaults to :math:`5000 ~ m`.
	...
	def update_topography(self, time):
		...
		Args:
			time (obj): :class:`datetime.timedelta` representing the elasped simulation time.
		"""
...
class SLEVE3d(XYZGrid):
	...
	def __init__(self, domain_x, nx, domain_y, ny, domain_z, nz, 
		...
		Args:
			...
			z_interface (`float`, optional): Interface value :math:`zmu_F = \mu_F`. If not 
				specified, it is assumed that :math:`\mu_F = \mu_T`, with :math:`\mu_T` the 
				value of :math:`\mu` at the top of the domain. In other words, a fully terrain-following 
