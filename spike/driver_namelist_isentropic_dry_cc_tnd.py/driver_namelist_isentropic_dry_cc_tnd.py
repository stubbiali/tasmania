...
if nl.isothermal:
	state = taz.get_isothermal_isentropic_state(
		...
		nl.init_temperature, moist=False, precipitation=False, 
		dtype=nl.dtype
	)
else:
	state = taz.get_default_isentropic_state(
		grid, nl.init_time, nl.init_x_velocity, nl.init_y_velocity,
		nl.init_brunt_vaisala, moist=False, precipitation=False, 
		dtype=nl.dtype
	)
...
for i in range(nt):
	...
	state_update = subtract(state_new, state, unshared_variables_in_output=True)
	...
	if (nl.print_frequency > 0) and ((i + 1) % nl.print_frequency == 0):
		...
		print(
			...
			'tnd_dyn_su = {:8.4f}'.format(
				i + 1, 
				np.linalg.norm(tnd_pg['x_momentum_isentropic'][20:61,20:61,:]),
				np.linalg.norm(tnd_cf['x_momentum_isentropic'][20:61,20:61,:]),
				np.linalg.norm(tnd_dyn['x_momentum_isentropic'][20:61,20:61,:]),
			)
		)
		print(
			...
			'tnd_dyn_sv = {:8.4f}'.format(
				i + 1, 
				np.linalg.norm(tnd_pg['y_momentum_isentropic'][20:61,20:61,:]),
				np.linalg.norm(tnd_cf['y_momentum_isentropic'][20:61,20:61,:]),
				np.linalg.norm(tnd_dyn['y_momentum_isentropic'][20:61,20:61,:]),
			)
		)
