...
from tasmania.python.grids.topography import Topography1d
...
class GridXZ:
    ...
    def __init__(
        ...
        # x-spacing
        dx_v = 1.0 if nx == 1 else (values_x[1] - values_x[0]) / (nx - 1.0)
        ...
        self.x_at_u_locations = sympl.DataArray(
            xv_u,
            coords=[xv_u],
            dims=dims_x,
        ...
        self.z_on_interface_levels = sympl.DataArray(
            zv_hl,
            coords=[zv_hl],
            dims=dims_z_hl,
        ...
        self.z = sympl.DataArray(
            zv, coords=[zv], dims=dims_z, name="z", attrs={"units": units_z}
        )
        ...
        if z_interface is None:
            self.z_interface = sympl.DataArray(
                values_z[0], attrs={"units": units_z}
            )
        else:
        ...
        if lt(values_z[0], values_z[1]):
            if not (
                le(values_z[0], z_interface_v)
                and le(z_interface_v, values_z[1])
            ):
                raise ValueError(
                    "z_interface should be in the range "
                    "({}, {}).".format(values_z[0], values_z[1])
                )
        else:
            if not (
                le(values_z[1], z_interface_v)
                and le(z_interface_v, values_z[0])
            ):
                raise ValueError(
                    "z_interface should be in the range "
        ...
        if topo_kwargs is None or not isinstance(topo_kwargs, dict):
            ...
            self.topography = Topography1d(
                self.x, topo_type=topo_type, topo_time=topo_time, **topo_kwargs
            )
