...
def plot_topography_3d(
    grid, state=None, field_to_plot="topography", fig=None, ax=None, **kwargs
):
    """
    ...
    out_fig, out_ax = plot_utils.get_figure_and_axes(
        fig, ax, figsize=figsize, fontsize=fontsize, projection="3d"
    )
    ...
    # Plot the topography
    surf = out_ax.plot_surface(xv, yv, zv, cmap=cm, linewidth=0.1)

    # Set the color bar
    if cbar_on:
        plot_utils.set_colorbar(
            out_fig,
            surf,
            color_levels,
