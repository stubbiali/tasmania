def make_animation_contourf_xy(grid, states_list, field_to_plot, z_level, save_dest, **kwargs):
	"""
	...
	**kwargs :
	...
	for t, state in enumerate(states_list):
		...
		if field_to_plot in state.keys():
			...
			try:
				rho, U, V = get_numpy_arrays(state, (slice(0,None), slice(0,None), z_level), 
			...
			try:
				s, U, V = get_numpy_arrays(state, (slice(0,None), slice(0,None), z_level), 
	...
	for t, state in enumerate(states_list):
		...
		if ni == nx and nj == ny:
			...
			topo_[1:-1,1:-1] = 0.25 * (_topo_[:-1,:-1] + _topo_[1:,:-1] +
									   _topo_[:-1,:1]  + _topo_[1:,1:])
...
def plot_animation_contourf_xy(time, x, y, field, topography, save_dest, **kwargs):
	"""
	...
	with writer.saving(fig, save_dest, nt):
		for n in range(nt):
			...
			plt.contour(x[:,:,n], y[:,:,n], topography[:,:,n], colors = 'gray')

			# Plot the field
			plt.contourf(x[:,:,n], y[:,:,n], field[:,:,n], color_scale, cmap = cm)
