...
def _create_dataarray(raw_array, grid, units = ''):
	...
	import sympl
	array = sympl.DataArray(raw_array, 
...
class _StateCreator():
	...
	def __call__(self, grid):
		...
		state['time'] = datetime(year = 1992, month = 2, day = 20) + timedelta(hours = self._counter)

		# Initialize density
		np.random.seed(19920220 + 10*self._counter)
		state['air_density'] = _create_dataarray(np.random.rand(nx, ny, nz), grid, units = 'kg m^-3')

		# Initialize x-velocity
		state['x_velocity_at_u_locations'] = _create_dataarray(np.random.rand(nx+1, ny, nz), grid, units = 'm s^-1')

		# Compute x-momentum
		rho = state['air_density'].values[:,:,:]
		u = 0.5 * (state['x_velocity_at_u_locations'].values[:-1,:,:] + state['x_velocity_at_u_locations'].values[1:,:,:])
		state['x_momentum'] = _create_dataarray(rho*u, grid, units = 'kg m^-2 s^-1')

		# Initialize y-velocity
		state['y_velocity_at_v_locations'] = _create_dataarray(np.random.rand(nx, ny+1, nz), grid, units = 'm s^-1')

		# Compute y-momentum
		v = 0.5 * (state['y_velocity_at_v_locations'].values[:,:-1,:] + state['y_velocity_at_v_locations'].values[:,1:,:])
		state['y_momentum'] = _create_dataarray(rho*v, grid, units = 'kg m^-2 s^-1')

		# Initialize pressure
		state['air_preassure_on_interface_levels'] = _create_dataarray(np.random.rand(nx, ny, nz+1), grid, units = 'Pa')

		# Initialize height
		raw_height = np.zeros((nx, ny, nz+1))
		raw_height[:,:,-1] = grid.topography_height
		for k in range(nz-1,-1,-1):
			raw_height[:,:,k] = raw_height[:,:,k+1] + 100.
		state['height_on_interface_levels'] = _create_dataarray(raw_height, grid, units = 'm')
