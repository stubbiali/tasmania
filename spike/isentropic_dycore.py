...
class SUSHomogeneousIsentropicDynamicalCore(SequentialSplittingDynamicalCore):
	...
	"""
	def __init__(self, grid, moist, time_integration_scheme,
				 horizontal_flux_scheme, horizontal_boundary_type,
				 damp=True, damp_type='rayleigh', damp_depth=15, damp_max=0.0002,
		...
		self._prognostic_x = HomogeneousIsentropicPrognostic.factory(
			time_integration_scheme, 'x', grid, moist, self._boundary,
			horizontal_flux_scheme, backend, dtype)
		self._prognostic_y = HomogeneousIsentropicPrognostic.factory(
			time_integration_scheme, 'y', grid, moist, self._boundary,
			horizontal_flux_scheme, backend, dtype)
...
class SSUSHomogeneousIsentropicDynamicalCore(SUSHomogeneousIsentropicDynamicalCore):
	...
	def __init__(self, grid, moist, time_integration_scheme,
		...
		"""
		super().__init__(
			grid, moist, time_integration_scheme,
			horizontal_flux_scheme, horizontal_boundary_type,
			damp, damp_type, damp_depth, damp_max,
			smooth, smooth_type, smooth_damp_depth,
			smooth_coeff, smooth_coeff_max,
			smooth_moist, smooth_moist_type,	smooth_moist_damp_depth,
