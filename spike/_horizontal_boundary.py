...
class _Periodic(HorizontalBoundary):
    ...
    def from_computational_to_physical_domain(
        self, phi_, out_dims=None, change_sign=False
    ):
        """
    ...
    def enforce(self, phi_new, phi_now=None, field_name=None, time=None):
        """
    ...
    def get_computational_grid(self):
        ...
        _domain_x = DataArray(
            [x.values[0] - nb * dx, x.values[-1] + nb * dx],
            dims=x.dims,
            attrs=x.attrs,
        )
        _nx = nx + 2 * nb
        ...
        _domain_y = DataArray(
            [y.values[0] - nb * dy, y.values[-1] + nb * dy],
            dims=y.dims,
            attrs=y.attrs,
        )
        _ny = ny + 2 * nb
        ...
        _domain_z = DataArray(
            [z_hl[0], z_hl[-1]], dims=self._grid.z.dims, attrs=z.attrs
        )
        _nz = nz
...
class _PeriodicXZ(HorizontalBoundary):
    ...
    def from_physical_to_computational_domain(self, phi):
        ...
        return np.concatenate(
            (
                np.repeat(phi_[:, 0:1, :], nb, axis=1),
                phi_,
                np.repeat(phi_[:, -1:, :], nb, axis=1),
            ),
            axis=1,
        )
    ...
    def get_computational_grid(self):
        ...
        # Determine the computational z-axis
        _domain_z = DataArray([z_hl[0], z_hl[-1]], dims=z.dims, attrs=z.attrs)
        _nz = nz
...
class _PeriodicYZ(HorizontalBoundary):
    ...
    def from_physical_to_computational_domain(self, phi):
        ...
        return np.concatenate(
            (
                np.repeat(phi_[0:1, :, :], nb, axis=0),
                phi_,
                np.repeat(phi_[-1:, :, :], nb, axis=0),
            ),
            axis=0,
        )
...
class _Relaxed(HorizontalBoundary):
    ...
    def __init__(self, grid, nb):
        ...
        rel[: min(nb, rel.shape[0])] = 1.0
        self._rel = rel[
            : min(min(int(grid.nx / 2), int(grid.ny / 2)), rel.shape[0])
        ]
        self._rrel = self._rel[::-1]
        self.nr = self._rel.size

        # The relaxation matrices for an unstaggered field
        self._s = np.repeat(
            self._rel[np.newaxis, :], grid.nx - 2 * self.nr, axis=0
        )
        self._n = np.repeat(
            self._rrel[np.newaxis, :], grid.nx - 2 * self.nr, axis=0
        )
        self._w = np.repeat(
            self._rel[:, np.newaxis], grid.ny - 2 * self.nr, axis=1
        )
        self._e = np.repeat(
            self._rrel[:, np.newaxis], grid.ny - 2 * self.nr, axis=1
        )

        # The relaxation matrices for a x-staggered field
        self._stgx_s = np.repeat(
            self._rel[np.newaxis, :], grid.nx - 2 * self.nr + 1, axis=0
        )
        self._stgx_n = np.repeat(
            self._rrel[np.newaxis, :], grid.nx - 2 * self.nr + 1, axis=0
        )

        # The relaxation matrices for a y-staggered field
        self._stgy_w = np.repeat(
            self._rel[:, np.newaxis], grid.ny - 2 * self.nr + 1, axis=1
        )
        self._stgy_e = np.repeat(
            self._rrel[:, np.newaxis], grid.ny - 2 * self.nr + 1, axis=1
        )
        ...
        self._s = np.concatenate((cnw, self._s, cne), axis=0)
        self._n = np.concatenate((csw, self._n, cse), axis=0)
        self._w = np.concatenate((cnw, self._w, cne), axis=1)
        self._e = np.concatenate((csw, self._e, cse), axis=1)
        ...
        self._s = np.repeat(self._s[:, :, np.newaxis], grid.nz + 1, axis=2)
        self._n = np.repeat(self._n[:, :, np.newaxis], grid.nz + 1, axis=2)
        self._w = np.repeat(self._w[:, :, np.newaxis], grid.nz + 1, axis=2)
        self._e = np.repeat(self._e[:, :, np.newaxis], grid.nz + 1, axis=2)
        self._stgx_s = np.repeat(
            self._stgx_s[:, :, np.newaxis], grid.nz + 1, axis=2
        )
        self._stgx_n = np.repeat(
            self._stgx_n[:, :, np.newaxis], grid.nz + 1, axis=2
        )
        self._stgy_w = np.repeat(
            self._stgy_w[:, :, np.newaxis], grid.nz + 1, axis=2
        )
        self._stgy_e = np.repeat(
            self._stgy_e[:, :, np.newaxis], grid.nz + 1, axis=2
        )
    ...
    def enforce(self, phi_new, phi_now, field_name=None, time=None):
        """
        ...
        if nj == ny:  # unstaggered
            ...
            phi_new[nb:nr, nb:-nb, :] -= self._w[nb:, nb:-nb, :nk] * (
                phi_new[nb:nr, nb:-nb, :] - west[nb:, nb:-nb, :]
            )
            phi_new[-nr:-nb, nb:-nb, :] -= self._e[:-nb, nb:-nb, :nk] * (
                phi_new[-nr:-nb, nb:-nb, :] - east[:-nb, nb:-nb, :]
            )
        else:  # y-staggered
            ...
            phi_new[nb:nr, nb:-nb, :] -= self._stgy_w[nb:, nb:-nb, :nk] * (
                phi_new[nb:nr, nb:-nb, :] - west[nb:, nb:-nb, :]
            )
            phi_new[-nr:-nb, nb:-nb, :] -= self._stgy_e[:-nb, nb:-nb, :nk] * (
                phi_new[-nr:-nb, nb:-nb, :] - east[:-nb, nb:-nb, :]
            )
        ...
        if ni == nx:  # unstaggered
            ...
            phi_new[nr:-nr, nb:nr, :] -= self._s[nr:-nr, nb:, :nk] * (
                phi_new[nr:-nr, nb:nr, :] - south[nr:-nr, nb:, :]
            )
            phi_new[nr:-nr, -nr:-nb, :] -= self._n[nr:-nr, :-nb, :nk] * (
                phi_new[nr:-nr, -nr:-nb, :] - north[nr:-nr, :-nb, :]
            )
        else:  # x-staggered
            ...
            phi_new[nr:-nr, nb:nr, :] -= self._stgx_s[nr:-nr, nb:, :nk] * (
                phi_new[nr:-nr, nb:nr, :] - south[nr:-nr, nb:, :]
            )
            phi_new[nr:-nr, -nr:-nb, :] -= self._stgx_n[nr:-nr, :-nb, :nk] * (
                phi_new[nr:-nr, -nr:-nb, :] - north[nr:-nr, :-nb, :]
            )
...
class _RelaxedXZ(HorizontalBoundary):
    ...
    def __init__(self, grid, nb):
        ...
        rel[: min(nb, rel.shape[0])] = 1.0
        self._rel = rel[: min(int(grid.nx / 2), rel.shape[0])]
        self._rrel = self._rel[::-1]
        self.nr = self._rel.size

        # The relaxation matrices
        self._stg_w = np.repeat(
            self._rel[:, np.newaxis, np.newaxis], grid.nz + 1, axis=2
        )
        self._stg_e = np.repeat(
            self._rrel[:, np.newaxis, np.newaxis], grid.nz + 1, axis=2
        )
    ...
    def from_physical_to_computational_domain(self, phi):
        ...
        return np.concatenate(
            (
                np.repeat(phi[:, 0:1, :], nb, axis=1),
                phi,
                np.repeat(phi[:, -1:, :], nb, axis=1),
            ),
            axis=1,
        )
    ...
    def enforce(self, phi_new, phi_now, field_name=None, time=None):
        ...
        phi_new[nb:nr, :, :] -= self._stg_w[nb:, :, :nk] * (
            phi_new[nb:nr, :, :] - west[nb:, :, :]
        )
        phi_new[-nr:-nb, :, :] -= self._stg_e[:-nb, :, :nk] * (
            phi_new[-nr:-nb, :, :] - east[:-nb, :, :]
        )
    ...
    def get_computational_grid(self):
        ...
        # Determine the computational x-axis
        _domain_x = DataArray(
            [x.values[0], x.values[-1]], dims=x.dims, attrs=x.attrs
        )
        _nx = nx

        # Determine the computational y-axis
        _domain_y = DataArray(
            [y.values[0] - nb * dy, y.values[0] + nb * dy],
            dims=y.dims,
            attrs=y.attrs,
        )
        _ny = ny + 2 * nb
...
class _RelaxedYZ(HorizontalBoundary):
    ...
    def __init__(self, grid, nb):
        ...
        rel[: min(nb, rel.shape[0])] = 1.0
        self._rel = rel[: min(int(grid.ny / 2), rel.shape[0])]
        self._rrel = self._rel[::-1]
        self.nr = self._rel.size

        # The relaxation matrices
        self._stg_s = np.repeat(
            self._rel[np.newaxis, :, np.newaxis], grid.nz + 1, axis=2
        )
        self._stg_n = np.repeat(
            self._rrel[np.newaxis, :, np.newaxis], grid.nz + 1, axis=2
        )
    ...
    def from_physical_to_computational_domain(self, phi):
        ...
        return np.concatenate(
            (
                np.repeat(phi[0:1, :, :], nb, axis=0),
                phi,
                np.repeat(phi[-1:, :, :], nb, axis=0),
            ),
            axis=0,
        )
    ...
    def enforce(self, phi_new, phi_now, field_name=None, time=None):
        ...
        phi_new[:, nb:nr, :] -= self._stg_s[:, nb:, :nk] * (
            phi_new[:, nb:nr, :] - south[:, nb:, :]
        )
        phi_new[:, -nr:-nb, :] -= self._stg_n[:, :-nb, :nk] * (
            phi_new[:, -nr:-nb, :] - north[:, :-nb, :]
        )
    ...
    def get_computational_grid(self):
        ...
        # Determine the computational x-axis
        _domain_x = DataArray(
            [x.values[0] - nb * dx, x.values[0] + nb * dx],
            dims=x.dims,
            attrs=x.attrs,
        )
        _nx = nx + 2 * nb
        ...
        # Determine the computational y-axis
        _domain_y = DataArray(
            [y.values[0], y.values[-1]], dims=y.dims, attrs=y.attrs
        )
        _ny = ny
...
class _RelaxedSymmetricXZ(_Relaxed):
    ...
    def from_computational_to_physical_domain(
        self, phi_, out_dims, change_sign=False
    ):
        """
        ...
        if out_dims[1] % 2 == 0:
            phi[:, half:, :] = (
                ...
                if change_sign
                else np.flip(phi[:, :half, :], axis=1)
            )
        else:
            phi[:, half:, :] = (
                -np.flip(phi[:, : half - 1, :], axis=1)
                if change_sign
                else np.flip(phi[:, : half - 1, :], axis=1)
            )
    ...
    def get_computational_grid(self):
        ...
        half = int((ny - 1) / 2)
        _domain_y = DataArray(
            [y.values[0], y.values[half + nb]], dims=y.dims, attrs=y.attrs
        )
        _ny = half + nb + 1
...
class _RelaxedSymmetricYZ(_Relaxed):
    ...
    def from_computational_to_physical_domain(
        ...
        if out_dims[0] % 2 == 0:
            phi[half:, :, :] = (
                -np.flip(phi[:half, :, :], axis=0)
                if change_sign
                else np.flip(phi[:half, :, :], axis=0)
            )
        else:
            phi[half:, :, :] = (
                -np.flip(phi[: half - 1, :, :], axis=0)
                if change_sign
    ...
    def get_computational_grid(self):
        ...
        half = int((nx - 1) / 2)
        _domain_x = DataArray(
            [x.values[0], x.values[half + nb]], dims=x.dims, attrs=x.attrs
        )
        _nx = half + nb + 1
