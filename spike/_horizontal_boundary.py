...
class _Relaxed(HorizontalBoundary):
	...
	def __init__(self, grid, nb):
		...
		rel[:min(nb, rel.shape[0])] = 1.
		self._rel = rel[:min(min(int(grid.nx/2), int(grid.ny/2)), rel.shape[0])]
		self._rrel = self._rel[::-1]
		self.nr = self._rel.size

		# The relaxation matrices for an unstaggered field
		self._s = np.repeat(self._rel[np.newaxis, :], grid.nx - 2*self.nr, axis=0)
		self._n = np.repeat(self._rrel[np.newaxis, :], grid.nx - 2*self.nr, axis=0)
		self._w = np.repeat(self._rel[:, np.newaxis], grid.ny - 2*self.nr, axis=1)
		self._e = np.repeat(self._rrel[:, np.newaxis], grid.ny - 2*self.nr, axis=1)
	...
	def enforce(self, phi_new, phi_now, field_name=None, time=None):
		...
		if nj == ny:  # unstaggered
			...
			phi_new[nb:nr, nb:-nb, :] -= \
				self._stgy_w[nb:, nb:-nb, :nk] * \
				(phi_new[nb:nr, nb:-nb, :] - west[nb:, nb:-nb, :])
			phi_new[-nr:-nb, nb:-nb, :] -= \
				self._stgy_e[:-nb, nb:-nb, :nk] * \
				(phi_new[-nr:-nb, nb:-nb, :] - east[:-nb, nb:-nb, :])
		...
		if ni == nx:  # unstaggered
			...
			phi_new[nr:-nr, nb:nr, :] -= \
				self._stgx_s[nr:-nr, nb:, :nk] * \
				(phi_new[nr:-nr, nb:nr, :] - south[nr:-nr, nb:, :])
			phi_new[nr:-nr, -nr:-nb, :] -= \
				self._stgx_n[nr:-nr, :-nb, :nk] * \
				(phi_new[nr:-nr, -nr:-nb, :] - north[nr:-nr, :-nb, :])
