...
from tasmania.dycore.horizontal_boundary_relaxed import (
    RelaxedSymmetricXZ,
    RelaxedSymmetricYZ,
)
...
class PrognosticIsentropicCentered(PrognosticIsentropic):
    ...
    moist isentropic_prognostic dynamical core.
    ...
    def __init__(
        ...
        moist_on,
        backend,
        coupling_physics_dynamics_on,
        sedimentation_on,
        sedimentation_flux_type,
        sedimentation_substeps,
    ):
        """
        Constructor.
        ...
        self._stencil_clipping_mass_fraction_and_diagnosing_isentropic_density_of_precipitation_water = (
            None
        )
    ...
    def step_neglecting_vertical_advection(
        self, dt, state, state_old=None, tendencies=None
    ):
        """
        ...
        s_new = self.boundary.from_computational_to_physical_domain(
        ...
        if type(self.boundary) == RelaxedSymmetricXZ:
            U_new = self.boundary.from_computational_to_physical_domain(
                self._out_U[:mi, :mj, :], (nx, ny, nz), change_sign=False
            )
            V_new = self.boundary.from_computational_to_physical_domain(
                self._out_V[:mi, :mj, :], (nx, ny, nz), change_sign=True
            )
        elif type(self.boundary) == RelaxedSymmetricYZ:
            U_new = self.boundary.from_computational_to_physical_domain(
                self._out_U[:mi, :mj, :], (nx, ny, nz), change_sign=True
            )
            V_new = self.boundary.from_computational_to_physical_domain(
                self._out_V[:mi, :mj, :], (nx, ny, nz), change_sign=False
            )
        else:
            U_new = self.boundary.from_computational_to_physical_domain(
                self._out_U[:mi, :mj, :], (nx, ny, nz)
            )
            V_new = self.boundary.from_computational_to_physical_domain(
                self._out_V[:mi, :mj, :], (nx, ny, nz)
            )
        ...
        Qv_new = (
            ...
            else self.boundary.from_computational_to_physical_domain(
                self._out_Qv[:mi, :mj, :], (nx, ny, nz)
            )
        )
        Qc_new = (
            None
        ...
        Qr_new = (
            ...
            if not self._moist_on
                ...
                self._out_Qr[:mi, :mj, :], (nx, ny, nz)
            )
        )

        # Apply the boundary conditions
        self.boundary.apply(
            s_new, state["air_isentropic_density"].values[:, :, :, 0]
        )
        self.boundary.apply(
            U_new, state["x_momentum_isentropic"].values[:, :, :, 0]
        )
        self.boundary.apply(
            V_new, state["y_momentum_isentropic"].values[:, :, :, 0]
        )
        if self._moist_on:
            self.boundary.apply(
                Qv_new,
            ...
            self.boundary.apply(
                Qc_new,
                state["cloud_liquid_water_isentropic_density"].values[
                    :, :, :, 0
                ],
            )
            self.boundary.apply(
                Qr_new,
                state["precipitation_water_isentropic_density"].values[
                    :, :, :, 0
                ],
            )

        # Instantiate the output state
        time = utils.convert_datetime64_to_datetime(
            state["air_isentropic_density"].coords["time"].values[0]
        )
        state_new = StateIsentropic(
            time + dt,
            self._grid,
        ...
        self._in_U_old[:mi, :mj, :] = self._in_U[:mi, :mj, :]
        self._in_V_old[:mi, :mj, :] = self._in_V[:mi, :mj, :]
        if self._moist_on:
    ...
    def step_integrating_sedimentation_flux(
        ...
        self._dts.value = (
            1.0e-6 * dts.microseconds if dts.seconds == 0.0 else dts.seconds
        )
        self._in_s[:nx, :ny, :] = self._in_s_old[:nx, :ny, :]
        self._in_s_prv[:nx, :ny, :] = state_prv[
            "air_isentropic_density"
        ].values[:, :, :, 0]
        self._in_qr[:, :, :] = (
            self._in_Qr_old[:nx, :ny, :] / self._in_s_old[:nx, :ny, :]
        )
        self._in_qr_prv[:, :, :] = state_prv[
            "mass_fraction_of_precipitation_water_in_air"
        ].values[:, :, :, 0]

        # Set the output fields
        nb = self._flux_sedimentation.nb
        self._out_s[:nx, :ny, :nb] = state_prv[
            "air_isentropic_density"
        ].values[:, :, :nb, 0]
        self._out_s[:nx, :ny, nb:] = self._in_s[:nx, :ny, nb:]
        self._out_qr[:, :, :nb] = state_prv[
            "mass_fraction_of_precipitation_water_in_air"
        ].values[:, :, :nb, 0]
        ...
        time_now = utils.convert_datetime64_to_datetime(
            state_now["air_density"].coords["time"].values[0]
        )
        state_new = StateIsentropic(
            time_now + dt,
            self._grid,
        ...
        if (
            ...
            and diagnostics["accumulated_precipitation"] is not None
        ):
            accumulated_precipitation[:, :, :] = diagnostics[
        ...
        self._in_s[:nx, :ny, :nb] = state_now["air_isentropic_density"].values[
            :, :, :nb, 0
        ]
        self._in_s[:nx, :ny, nb:] = self._out_s[:nx, :ny, nb:]
        self._in_qr[:, :, :nb] = state_now[
            "mass_fraction_of_precipitation_water_in_air"
        ].values[:, :, :nb, 0]
        ...
        # Shortcuts
        p_ = (
            state_now["air_pressure"]
            if state_now["air_pressure"] is not None
        ...
        for n in range(self._sedimentation_substeps):
            ...
            state_new.update(
                self.diagnostic.get_height(state_new, pt=p_.values[0, 0, 0, 0])
            )
            ...
            self._in_vt[
                :, :, :
            ] = self.microphysics.get_raindrop_fall_velocity(state_new)
            ...
            # Note: the precipitation is accumulated only on the time interval from the current to the next time level
            ppt = (
                self._in_rho[:, :, -1:]
                * self._in_qr[:, :, -1:]
                * self._in_vt[:, :, -1:]
            ...
            if n >= self._sedimentation_substeps / 2:
                accumulated_precipitation[:, :, :] += ppt[:, :, :] * 1.0e3
    ...
    def _stencil_stepping_by_neglecting_vertical_advection_initialize(
        ...
        tendencies : obj
                ...
                * tendency_of_mass_fraction_of_precipitation_water_in_air (unstaggered).
        """
        # Infer the shape of any (unstaggered) input field
        s = self.boundary.from_physical_to_computational_domain(
            state["air_isentropic_density"].values[:, :, :, 0]
        )
        mi, mj, nz = s.shape

        # Allocate the attributes which will serve as inputs to the stencil
        self._stencils_stepping_by_neglecting_vertical_advection_allocate_inputs(
            mi, mj, tendencies
        )
        ...
        if self._moist_on:
            _inputs.update(
                {
                    "in_Qv": self._in_Qv,
                    "in_Qv_old": self._in_Qv_old,
                    "in_Qc": self._in_Qc,
                    "in_Qc_old": self._in_Qc_old,
                    "in_Qr": self._in_Qr,
                    "in_Qr_old": self._in_Qr_old,
                }
            )
            _outputs.update(
                {
        ...
        if tendencies is not None:
            if (
                tendencies["tendency_of_mass_fraction_of_water_vapor_in_air"]
                is not None
            ):
    ...
    def _stencils_stepping_by_neglecting_vertical_advection_allocate_inputs(
        ...
        super()._stencils_stepping_by_neglecting_vertical_advection_allocate_inputs(
            mi, mj, tendencies
        )
        ...
        li = (
            mi
            if not (
                self._sedimentation_on and self._physics_dynamics_coupling_on
            )
            else max(mi, nx)
        )
        lj = (
    ...
    def _stencils_stepping_by_neglecting_vertical_advection_set_inputs(
        ...
        if state_old is not None:  # Old state explicitly given
            self._in_s_old[
                :, :, :
            ] = self.boundary.from_physical_to_computational_domain(
            ...
            if self._moist_on:
                self._in_Qv_old[
                    ...
                    state_old["water_vapor_isentropic_density"].values[
                        :, :, :, 0
                    ]
                )
                self._in_Qc_old[
                    ...
                    state_old["cloud_liquid_water_isentropic_density"].values[
                        :, :, :, 0
                    ]
                )
                self._in_Qr_old[
                    ...
                    state_old["precipitation_water_isentropic_density"].values[
                        :, :, :, 0
                    ]
                )
    ...
    def _stencil_stepping_by_neglecting_vertical_advection_defs(
        ...
        in_Qv : `obj`, optional
                :class:`gridtools.Equation` representing the isentropic_prognostic density of water vapour at the current time.
        in_Qc : `obj`, optional
                :class:`gridtools.Equation` representing the isentropic_prognostic density of cloud water at the current time.
        in_Qr : `obj`, optional
                :class:`gridtools.Equation` representing the isentropic_prognostic density of precipitation water at the current time.
        in_Qv_old : `obj`, optional
                :class:`gridtools.Equation` representing the isentropic_prognostic density of water vapour at the previous time level.
        in_Qc_old : `obj`, optional
                :class:`gridtools.Equation` representing the isentropic_prognostic density of cloud water at the previous time level.
        in_Qr_old : `obj`, optional
                :class:`gridtools.Equation` representing the isentropic_prognostic density of precipitation water at the previous time level.
        ...
        if not self._moist_on:
            (
                ...
                flux_V_x,
                flux_V_y,
            ) = self._flux.get_horizontal_fluxes(
                i, j, k, dt, in_s, in_u, in_v, in_mtg, in_U, in_V
            )
        else:
            (
                flux_s_x,
        ...
        out_s[i, j, k] = in_s_old[i, j, k] - 2.0 * dt * (
            (flux_s_x[i, j, k] - flux_s_x[i - 1, j, k]) / self._grid.dx
        ...
        out_U[i, j, k] = (
            ...
            * dt
            * (
                (flux_U_x[i, j, k] - flux_U_x[i - 1, j, k]) / self._grid.dx
                + (flux_U_y[i, j, k] - flux_U_y[i, j - 1, k]) / self._grid.dy
            )
            - dt
            * in_s[i, j, k]
            * (in_mtg[i + 1, j, k] - in_mtg[i - 1, j, k])
            / self._grid.dx
        )
        ...
        out_V[i, j, k] = (
            ...
            * dt
            * (
                (flux_V_x[i, j, k] - flux_V_x[i - 1, j, k]) / self._grid.dx
                + (flux_V_y[i, j, k] - flux_V_y[i, j - 1, k]) / self._grid.dy
            )
            - dt
            * in_s[i, j, k]
            * (in_mtg[i, j + 1, k] - in_mtg[i, j - 1, k])
            / self._grid.dy
        )
        ...
        if self._moist_on:
            ...
            if in_qv_tnd is None:
                out_Qv[i, j, k] = in_Qv_old[i, j, k] - 2.0 * dt * (
                    (flux_Qv_x[i, j, k] - flux_Qv_x[i - 1, j, k])
                    / self._grid.dx
                    + (flux_Qv_y[i, j, k] - flux_Qv_y[i, j - 1, k])
                    / self._grid.dy
                )
            else:
                out_Qv[i, j, k] = in_Qv_old[i, j, k] - 2.0 * dt * (
            ...
            if in_qc_tnd is None:
                out_Qc[i, j, k] = in_Qc_old[i, j, k] - 2.0 * dt * (
                    (flux_Qc_x[i, j, k] - flux_Qc_x[i - 1, j, k])
                    / self._grid.dx
                    + (flux_Qc_y[i, j, k] - flux_Qc_y[i, j - 1, k])
                    / self._grid.dy
                )
            else:
                out_Qc[i, j, k] = in_Qc_old[i, j, k] - 2.0 * dt * (
            ...
            if in_qr_tnd is None:
                out_Qr[i, j, k] = in_Qr_old[i, j, k] - 2.0 * dt * (
                    (flux_Qr_x[i, j, k] - flux_Qr_x[i - 1, j, k])
                    / self._grid.dx
                    + (flux_Qr_y[i, j, k] - flux_Qr_y[i, j - 1, k])
                    / self._grid.dy
                )
            else:
                out_Qr[i, j, k] = in_Qr_old[i, j, k] - 2.0 * dt * (
    ...
    def _stencil_stepping_by_coupling_physics_with_dynamics_defs(
        ...
        # Computations
        if not self._moist_on:
            flux_s_z, flux_U_z, flux_V_z = self._flux.get_vertical_fluxes(
                i,
                j,
                k,
                dt,
            ...
            (
                ...
                flux_Qc_z,
                flux_Qr_z,
            ) = self._flux.get_vertical_fluxes(
                i,
                j,
                k,
                dt,
        ...
        out_s[i, j, k] = (
            ...
            - 2.0
            * dt
            * (flux_s_z[i, j, k] - flux_s_z[i, j, k + 1])
            / self._grid.dz
        )
        out_U[i, j, k] = (
            in_U_prv[i, j, k]
            - 2.0
            * dt
            * (flux_U_z[i, j, k] - flux_U_z[i, j, k + 1])
            / self._grid.dz
        )
        out_V[i, j, k] = (
            in_V_prv[i, j, k]
            - 2.0
            * dt
            * (flux_V_z[i, j, k] - flux_V_z[i, j, k + 1])
            / self._grid.dz
        )
        if self._moist_on:
            out_Qv[i, j, k] = (
                ...
                - 2.0
                * dt
                * (flux_Qv_z[i, j, k] - flux_Qv_z[i, j, k + 1])
                / self._grid.dz
            )
            out_Qc[i, j, k] = (
                in_Qc_prv[i, j, k]
                - 2.0
                * dt
                * (flux_Qc_z[i, j, k] - flux_Qc_z[i, j, k + 1])
                / self._grid.dz
            )
            out_Qr[i, j, k] = (
                in_Qr_prv[i, j, k]
                - 2.0
                * dt
                * (flux_Qr_z[i, j, k] - flux_Qr_z[i, j, k + 1])
                / self._grid.dz
            )
    ...
    def _stencils_stepping_by_integrating_sedimentation_flux_initialize(self):
        ...
        self._stencil_computing_slow_tendencies = gt.NGStencil(
            ...
            inputs={
                "in_s_old": self._in_s,
                "in_s_prv": self._in_s_prv,
                "in_qr_old": self._in_qr,
                "in_qr_prv": self._in_qr_prv,
            },
            global_inputs={"dt": self._dt},
            outputs={
                "out_s_tnd": self._tmp_s_tnd,
                "out_qr_tnd": self._tmp_qr_tnd,
            },
        ...
        self._stencil_stepping_by_integrating_sedimentation_flux = gt.NGStencil(
            ...
            inputs={
                ...
                "in_qr": self._in_qr,
                "in_vt": self._in_vt,
                "in_s_tnd": self._tmp_s_tnd,
                "in_qr_tnd": self._tmp_qr_tnd,
            },
            global_inputs={"dts": self._dts},
            outputs={
                "out_s": self._out_s[:nx, :ny, :],
                "out_qr": self._out_qr,
            },
        ...
        self._stencil_clipping_mass_fraction_and_diagnosing_isentropic_density_of_precipitation_water = gt.NGStencil(
            ...
            inputs={"in_s": self._in_s, "in_qr": self._in_qr},
            outputs={
                "out_qr": self._out_qr,
                "out_Qr": self._out_Qr[:nx, :ny, :],
            },
            domain=gt.domain.Rectangle((0, 0, 0), (nx - 1, ny - 1, nz - 1)),
            mode=self._backend,
        )

    def _stencil_computing_slow_tendencies_defs(
        self, dt, in_s_old, in_s_prv, in_qr_old, in_qr_prv
    ):
        """
        ...
        out_qr_tnd[i, j, k] = (
            0.5 * (in_qr_prv[i, j, k] - in_qr_old[i, j, k]) / dt
        )
    ...
    def _stencil_ensuring_vertical_cfl_is_obeyed_defs(self, dts, in_h, in_vt):
        ...
        # Computations
        tmp_cfl[i, j, k] = (
            in_vt[i, j, k] * dts / (in_h[i, j, k] - in_h[i, j, k + 1])
        )
        out_vt[i, j, k] = (tmp_cfl[i, j, k] < 0.95) * in_vt[i, j, k] + (
            tmp_cfl[i, j, k] > 0.95
        ) * 0.95 * (in_h[i, j, k] - in_h[i, j, k + 1]) / dts
    ...
    def _stencil_stepping_by_integrating_sedimentation_flux_defs(
        ...
        tmp_dfdz = self._flux_sedimentation.get_vertical_derivative_of_sedimentation_flux(
            i, j, k, in_rho, in_h, in_qr, in_vt
        )
        tmp_qr_st[i, j, k] = in_qr[i, j, k] + dts * in_qr_tnd[i, j, k]
        tmp_qr[i, j, k] = (
            tmp_qr_st[i, j, k] + dts * tmp_dfdz[i, j, k] / in_rho[i, j, k]
        )
        out_qr[i, j, k] = (tmp_qr[i, j, k] > 0.0) * tmp_qr[i, j, k] + (
            tmp_qr[i, j, k] < 0.0
        ) * tmp_qr_st[i, j, k]

        return out_s, out_qr

    def _stencil_clipping_mass_fraction_and_diagnosing_isentropic_density_of_precipitation_water_defs(
        self, in_s, in_qr
    ):
        """
        ...
        # Computations
        out_qr[i, j, k] = (in_qr[i, j, k] > 0.0) * in_qr[i, j, k]
