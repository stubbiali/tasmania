    ...
    **kwargs :
    ...
    try:
        ...
        try:
            ...
            try:
                zv = get_numpy_arrays(
                    state, (slice(0, None), y_level, slice(0, None)), "height"
                )
...
def plot_grid_yz(grid, state, field_to_plot, x_level, fig, ax, **kwargs):
    ...
    **kwargs :
    ...
    try:
        zv = get_numpy_arrays(
            state,
            (x_level, slice(0, None), slice(0, None)),
            "height_on_interface_levels",
        )
        topo = np.copy(zv[:, -1])
    except KeyError:
        try:
            ...
            try:
                zv = get_numpy_arrays(
                    state, (x_level, slice(0, None), slice(0, None)), "height"
                )
                topo = grid.topography_height
            except KeyError:
                try:
...
def plot_grid_vertical_section(
    hor_grid, vert_grid, topography, fig, ax, **kwargs
):
    """
    ...
    hor_factor = kwargs.get("hor_factor", 1.0)
    vert_factor = kwargs.get("vert_factor", 1.0)
    ...
    # Plot the topography
    ax.plot(
        hor_grid[:, -1], topography, color="black", linewidth=linewidth + 0.5
    )
