...
def convert_old_to_new_pickle(
    filename, filename_new=None, apply_name_conversion=True
):
    ...
    with open(filename, "rb") as data:
        ...
        for t in range(nt):
            ...
            for state_name in state_names:
                state_name_new = (
                    convert_variable_name(state_name)
                    if apply_name_conversion
                    else state_name
                )
                state[state_name_new] = states_old[state_name][:, :, :, t]
            for diagnostic_name in diagnostics_names:
                diagnostic_name_new = (
                    convert_variable_name(diagnostic_name)
                    if apply_name_conversion
                    else diagnostic_name
                )
                try:
                    state[diagnostic_name_new] = diagnostics_old[
                        diagnostic_name
                    ].loc[:, :, :, current_time]
                except IndexError:
...
def convert_grid(grid):
    domain_x = DataArray(
        [grid.x.values[0], grid.x.values[-1]],
        dims="x",
        attrs={"units": grid.x.attrs["units"]},
    )
    nx = grid.nx
    domain_y = DataArray(
        [grid.y.values[0], grid.y.values[-1]],
        dims="y",
        attrs={"units": grid.y.attrs["units"]},
    )
    ny = grid.ny
    domain_z = DataArray(
        [grid.z_half_levels.values[0], grid.z_half_levels.values[-1]],
    ...
    try:
        ...
        topo_kwargs = {
            "topo_max_height": DataArray(
                topo_kwargs_["topo_max_height"], attrs={"units": "m"}
            ),
            "topo_width_x": DataArray(
                topo_kwargs_["topo_width_x"], attrs={"units": "m"}
            ),
            "topo_width_y": DataArray(
                topo_kwargs_["topo_width_y"], attrs={"units": "m"}
            ),
            "topo_center_x": DataArray(
                topo_kwargs_["topo_center_x"], attrs={"units": "m"}
            ),
            "topo_center_y": DataArray(
                topo_kwargs_["topo_center_y"], attrs={"units": "m"}
            ),
        }
    except AttributeError:
        try:
        ...
        for state_ in states:
            ...
            for key_ in state_.keys():
                if key_ != "time":
                    ...
                    raw_array = (
                        np.zeros(get_shape(key, grid))
                        if state_[key_] is None
                        else state_[key_].values
                    )
                    units = (
                        get_default_units(key)
                        if state_[key_] is None
                        else state_[key_].attrs["units"]
                    )

                    state[key] = make_data_array_3d(
                        raw_array, grid, units, name=key
                    )
