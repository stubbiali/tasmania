...
from spike.prognostic import IsentropicPrognostic
...
class Centered(IsentropicPrognostic):
    ...
    prognostic part of the three-dimensional moist isentropic_prognostic dynamical core.
    """
    ...
    def __init__(
        ...
        adiabatic_flow=True,
        vertical_flux_scheme=None,
        sedimentation_on=False,
        sedimentation_flux_scheme=None,
        sedimentation_substeps=2,
        raindrop_fall_velocity_diagnostic=None,
        ...
        super().__init__(
            ...
            sedimentation_on,
            sedimentation_flux_scheme,
            sedimentation_substeps,
            raindrop_fall_velocity_diagnostic,
            backend,
            dtype,
            physical_constants,
        )
    ...
    def step_neglecting_vertical_motion(
        self, stage, dt, raw_state, raw_tendencies=None
    ):
        """
        ...
        if self._moist_on:
            self._hboundary.enforce(
                sqv_new, raw_state["isentropic_density_of_water_vapor"]
            )
            self._hboundary.enforce(
                sqc_new, raw_state["isentropic_density_of_cloud_liquid_water"]
            )
            self._hboundary.enforce(
                sqr_new, raw_state["isentropic_density_of_precipitation_water"]
            )
    ...
    def step_integrating_vertical_advection(
        self, stage, dt, raw_state_now, raw_state_prv
    ):
        """
        Method advancing the conservative, prognostic model variables
    ...
    def step_integrating_sedimentation_flux(
        ...
        self._water_constituent_diagnostic.get_mass_fraction_of_water_constituent_in_air(
            self._in_s_old[:nx, :ny, :],
            self._in_sqr_old[:nx, :ny, :],
            self._in_qr,
        )
        ...
        self._in_s[:nx, :ny, nb:] = raw_state_now["air_isentropic_density"][
            :, :, nb:
        ]
        self._in_qr[:, :, nb:] = raw_state_now[mf_pw][:, :, nb:]

        # Set the upper layers of the output fields
        self._out_s[:nx, :ny, :nb] = raw_state_prv["air_isentropic_density"][
            :, :, :nb
        ]
        ...
        if raw_state_now.get("accumulated_precipitation", None) is not None:
            accumulated_precipitation[...] = raw_state_now[
        ...
        for n in range(self._sedimentation_substeps):
            ...
            self._in_h[...] = self._diagnostics.get_height(
                self._in_s[:nx, :ny, :], pt
            )

            # Diagnose the air density
            self._in_rho[...] = self._diagnostics.get_air_density(
                self._in_s[:nx, :ny, :], self._in_h
            )

            # Compute the raindrop fall velocity
            vt_dict = self._fall_velocity_diagnostic(state_new)
            self._in_vt[...] = (
                vt_dict["raindrop_fall_velocity"]
                .to_units("m s^-1")
                .values[...]
            )
    ...
    def _stencil_stepping_by_neglecting_vertical_motion_initialize(
        ...
        self._stencils_stepping_by_neglecting_vertical_motion_allocate_inputs(
            raw_tendencies
        )
    ...
    def _stencils_stepping_by_neglecting_vertical_motion_allocate_inputs(
        self, raw_tendencies
    ):
        """
        ...
        super()._stencils_stepping_by_neglecting_vertical_motion_allocate_inputs(
            raw_tendencies
        )
    ...
    def _stencil_stepping_by_neglecting_vertical_motion_defs(
        ...
        in_s : obj
                :class:`gridtools.Equation` representing the isentropic_prognostic density
                at the current time.
        in_u : obj
        ...
        in_sv_old : obj
                ...
                at the previous time level.
        in_sqv : `obj`, optional
                :class:`gridtools.Equation` representing the isentropic_prognostic density of
    ...
    def _stencil_computing_slow_tendencies_defs(
        dt, in_s_old, in_s_prv, in_qr_old, in_qr_prv
    ):
        """
        GT4Py stencil computing the slow tendencies of isentropic_prognostic density
        ...
        out_s_tnd : obj :
                ...
                of the isentropic_prognostic density.
        out_qr_tnd : obj :
                :class:`gridtools.Equation` representing the slow tendency
    ...
    def _stencil_stepping_by_integrating_sedimentation_flux_defs(
        ...
        """
        GT4Py stencil stepping the isentropic_prognostic density and the mass fraction
        ...
        out_qr[i, j, k] = (tmp_qr[i, j, k] > 0.0) * tmp_qr[i, j, k] + (
            tmp_qr[i, j, k] < 0.0
        ) * ((tmp_qr_st[i, j, k] > 0.0) * tmp_qr_st[i, j, k])
...
class ForwardEuler(IsentropicPrognostic):
    ...
    def step_neglecting_vertical_motion(
        ...
        # Run the compute function of the stencil stepping the isentropic_prognostic density
        ...
        if self._hflux_scheme in ["upwind", "centered", "fifth_order_upwind"]:
            ...
            s_prov = 0.5 * (raw_state["air_isentropic_density"] + s_new)
        else:
        ...
        _, _, mtg_prv, _ = self._diagnostics.get_diagnostic_variables(
            s_prov, pt
        )

        # Extend the updated isentropic_prognostic density and Montgomery potential to
    ...
    def _stencils_stepping_by_neglecting_vertical_motion_allocate_temporaries(
        self,
    ):
        """
    ...
    def _stencil_stepping_by_neglecting_vertical_motion_first_defs(
        ...
        """
        GT4Py stencil stepping the isentropic_prognostic density and the water constituents
        ...
        out_sqv : `obj`, optional
                :class:`gridtools.Equation` representing the stepped
                isentropic_prognostic density of water vapour.
        out_sqc : `obj`, optional
                :class:`gridtools.Equation` representing the stepped
                isentropic_prognostic density of cloud water.
        out_sqr : `obj`, optional
                :class:`gridtools.Equation` representing the stepped
                isentropic_prognostic density of precipitation water.
        """
        # Shortcuts
        dx = self._grid.dx.to_units("m").values.item()
        dy = self._grid.dy.to_units("m").values.item()
    ...
    def _stencil_stepping_by_neglecting_vertical_motion_second_defs(
        ...
        in_mtg : obj
                ...
                potential diagnosed from the stepped isentropic_prognostic density.
        in_su : obj
                :class:`gridtools.Equation` representing the provisional
                :math:`x`-momentum.
        in_sv : obj
    ...
    @staticmethod
    def _stencil_computing_slow_tendencies_defs(
        dt, in_s, in_s_prv, in_qr, in_qr_prv
    ):
        """
        GT4Py stencil computing the slow tendencies of isentropic_prognostic density
        and mass fraction of precipitation rain.
...
class RK2(IsentropicPrognostic):
    ...
    def _stencils_stepping_by_neglecting_vertical_motion_set_inputs(
        ...
        if raw_tendencies is not None:
            s_tnd_on = (
                raw_tendencies.get("air_isentropic_density", None) is not None
            )
            ...
            su_tnd_on = (
                raw_tendencies.get("x_momentum_isentropic", None) is not None
            )
            sv_tnd_on = (
                raw_tendencies.get("y_momentum_isentropic", None) is not None
            )
        else:
            s_tnd_on = (
                su_tnd_on
    ...
    def _get_pressure_gradient(self, i, j, in_mtg):
        ...
        if self._hflux_scheme in ["fifth_order_upwind"]:
            ...
            pgx[i, j] = (in_mtg[i + 1, j] - in_mtg[i - 1, j]) / (2.0 * dx)
            pgy[i, j] = (in_mtg[i, j + 1] - in_mtg[i, j - 1]) / (2.0 * dy)
...
class RK3(IsentropicPrognostic):
    ...
    def _stencil_stepping_by_neglecting_vertical_motion_second_defs(
        ...
        out_s[i, j] = (
            in_s[i, j] + b21 * in_s0[i, j] + (a2 - b21) * out_s1[i, j]
        )
        ...
        out_su[i, j] = (
            in_su[i, j] + b21 * in_su0[i, j] + (a2 - b21) * out_su1[i, j]
        )
        ...
        out_sv[i, j] = (
            in_sv[i, j] + b21 * in_sv0[i, j] + (a2 - b21) * out_sv1[i, j]
        )
