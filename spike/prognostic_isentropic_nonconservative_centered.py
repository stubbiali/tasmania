...
class PrognosticIsentropicNonconservativeCentered(
    PrognosticIsentropicNonconservative
):
    """
    ...
    def __init__(
        ...
        :meth:`~tasmania.dycore.prognostic_isentropic_nonconservative.PrognosticIsentropicNonconservative.factory` of
        ...
        self._stencil_stepping_by_neglecting_vertical_advection_unstaggered = (
            None
        )
        self._stencil_stepping_by_neglecting_vertical_advection_velocity_x = (
            None
        )
        self._stencil_stepping_by_neglecting_vertical_advection_velocity_y = (
            None
        )
    ...
    def step_neglecting_vertical_advection(
        ...
        if type(self.boundary) == RelaxedSymmetricXZ:
            ...
            u_new = self.boundary.from_computational_to_physical_domain(
                self._out_u[: mi + 1, :mj, :], (nx + 1, ny, nz)
            )
            v_new = self.boundary.from_computational_to_physical_domain(
                self._out_v[:mi, : mj + 1, :], (nx, ny + 1, nz)
            )
        ...
        if self._moist_on:
            self.boundary.apply(
                qv_new,
                state["mass_fraction_of_water_vapor_in_air"].values[
                    :, :, :, 0
                ],
            )
            self.boundary.apply(
                qc_new,
                state["mass_fraction_of_cloud_liquid_water_in_air"].values[
                    :, :, :, 0
                ],
            )
            self.boundary.apply(
                qr_new,
                state["mass_fraction_of_precipitation_water_in_air"].values[
                    :, :, :, 0
                ],
            )
    ...
    def _stencils_stepping_by_neglecting_vertical_advection_set_inputs(
        ...
        if state_old is not None:  # Old state explicitly given
            ...
            self._in_u_old[
                ...
                state_old["x_velocity"].values[:, :, :, 0]
            )
            self._in_v_old[
                :mi, : mj + 1, :
            ] = self.boundary.from_physical_to_computational_domain(
                state_old["y_velocity"].values[:, :, :, 0]
            )
            ...
            if self._moist_on:
                self._in_qv_old[
                    ...
                    state_old[qv_label].values[:, :, :, 0]
                )
                self._in_qc_old[
                    :mi, :mj, :
                ] = self.boundary.from_physical_to_computational_domain(
                    state_old[qc_label].values[:, :, :, 0]
                )
                self._in_qr_old[
                    :mi, :mj, :
                ] = self.boundary.from_physical_to_computational_domain(
                    state_old[qr_label].values[:, :, :, 0]
                )
            ...
            self._in_u_old[
                ...
                state["x_velocity"].values[:, :, :, 0]
            )
            self._in_v_old[
                :mi, : mj + 1, :
            ] = self.boundary.from_physical_to_computational_domain(
                state["y_velocity"].values[:, :, :, 0]
            )
            ...
            if self._moist_on:
                self._in_qv_old[
                    ...
                    state[qv_label].values[:, :, :, 0]
                )
                self._in_qc_old[
                    :mi, :mj, :
                ] = self.boundary.from_physical_to_computational_domain(
                    state[qc_label].values[:, :, :, 0]
                )
                self._in_qr_old[
                    :mi, mj:, :
                ] = self.boundary.from_physical_to_computational_domain(
                    state[qr_label].values[:, :, :, 0]
                )

    def _stencil_stepping_by_neglecting_vertical_advection_unstaggered_defs(
        self,
        dt,
        in_s,
        in_u,
        ...
        if not self._moist_on:
            flux_s_x, flux_s_y, _, _, _, _ = self._flux.get_horizontal_fluxes(
                i, j, k, dt, in_s, in_u, in_v, in_mtg
            )
        else:
            (
                flux_s_x,
                ...
                i, j, k, dt, in_s, in_u, in_v, in_mtg, in_qv, in_qc, in_qr
            )
        ...
        out_s[i, j, k] = in_s_old[i, j, k] - 2.0 * dt * (
            (flux_s_x[i + 1, j, k] - flux_s_x[i, j, k]) / dx
            + (flux_s_y[i, j + 1, k] - flux_s_y[i, j, k]) / dy
        )

        if self._moist_on:
            # Step the mass fraction of water vapor
            out_qv[i, j, k] = in_qv_old[i, j, k] - dt * (
                tmp_u[i, j, k]
                * (flux_qv_x[i + 1, j, k] - flux_qv_x[i, j, k])
                / dx
                + tmp_v[i, j, k]
                * (flux_qv_y[i, j + 1, k] - flux_qv_y[i, j, k])
                / dy
            )

            # Step the mass fraction of cloud liquid water
            out_qc[i, j, k] = in_qc_old[i, j, k] - dt * (
                tmp_u[i, j, k]
                * (flux_qc_x[i + 1, j, k] - flux_qc_x[i, j, k])
                / dx
                + tmp_v[i, j, k]
                * (flux_qc_y[i, j + 1, k] - flux_qc_y[i, j, k])
                / dy
            )

            # Step the mass fraction of precipitation water
            out_qr[i, j, k] = in_qr_old[i, j, k] - dt * (
                tmp_u[i, j, k]
                * (flux_qr_x[i + 1, j, k] - flux_qr_x[i, j, k])
                / dx
                + tmp_v[i, j, k]
                * (flux_qr_y[i, j + 1, k] - flux_qr_y[i, j, k])
                / dy
            )
    ...
    def _stencil_stepping_by_neglecting_vertical_advection_velocity_x_defs(
        self, dt, in_s, in_u, in_v, in_mtg, in_u_old
    ):
        """
        ...
        # Evaluate the numerical fluxes
        _, _, flux_u_x, flux_u_y, _, _ = self._flux.get_horizontal_fluxes(
            i, j, k, dt, in_s, in_u, in_v, in_mtg
        )

        # Step the x-velocity
        out_u[i, j, k] = in_u_old[i, j, k] - 2.0 * dt * (
            in_u[i, j, k] * (flux_u_x[i, j, k] - flux_u_x[i - 1, j, k]) / dx
            + 0.25
            * (
                in_v[i - 1, j, k]
                + in_v[i, j, k]
            ...
            * (flux_u_y[i, j + 1, k] - flux_u_y[i, j, k])
            / dy
    ...
    def _stencil_stepping_by_neglecting_vertical_advection_velocity_y_defs(
        self, dt, in_s, in_u, in_v, in_mtg, in_v_old
    ):
        """
        ...
        # Evaluate the numerical fluxes
        _, _, _, _, flux_v_x, flux_v_y = self._flux.get_horizontal_fluxes(
            i, j, k, dt, in_s, in_u, in_v, in_mtg
        )

        # Step the y-velocity
        out_v[i, j, k] = in_v_old[i, j, k] - 2.0 * dt * (
            0.25
            * (
                in_u[i, j - 1, k]
                + in_u[i + 1, j - 1, k]
            ...
            * (flux_v_x[i + 1, j, k] - flux_v_x[i, j, k])
            / dx
