...
#
def make_animation_contourf_xz(
    grid, states_list, field_to_plot, y_level, save_dest, **kwargs
):
    """
	...
	**kwargs :
    ...
    for t, state in enumerate(states_list):
        ...
        if field_to_plot in state.keys():
            ...
            var_ = (
                u * (h_mid_at_u_loc[1:, :] - h_mid_at_u_loc[:-1, :]) / grid.dx
            )
        else:
...
def plot_animation_contourf_xz(
    time, x, z, field, topography, save_dest, **kwargs
):
    """
    ...
    with writer.saving(fig, save_dest, nt):
        for n in range(nt):
            ...
            # Plot the field
            plt.contourf(
                x[:, :, n], z[:, :, n], field[:, :, n], color_scale, cmap=cm
            )
