...
class PrognosticIsentropicForwardEuler(PrognosticIsentropic):
    ...
    def step_neglecting_vertical_advection(
        ...
        if (
            self._stencil_stepping_by_neglecting_vertical_advection_first
            is None
        ):
        ...
        out_s = self.boundary.from_computational_to_physical_domain(
        ...
        self.boundary.apply(
            out_s, state["air_isentropic_density"].values[:, :, :, 0]
        )
        if self._moist_on:
            self.boundary.apply(
                out_Qv,
        ...
        if self._flux_scheme in ["upwind", "centered"]:
            ...
            s_prov = 0.5 * (
                state["air_isentropic_density"].values[:, :, :, 0] + out_s
            )
        ...
        state_prov = StateIsentropic(
            time + 0.5 * dt, self._grid, air_isentropic_density=s_prov
        )
        p_ = (
            state["air_pressure"]
        ...
        gd = self.diagnostic.get_diagnostic_variables(
            state_prov, p_.values[0, 0, 0, 0]
        )
        ...
        self._in_s_prv[
            :mi, :mj, :
        ] = self.boundary.from_physical_to_computational_domain(s_prov)
        self._in_mtg_prv[
            :mi, :mj, :
        ...
        if type(self.boundary) == RelaxedSymmetricXZ:
            out_U = self.boundary.from_computational_to_physical_domain(
                self._out_U[:mi, :mj, :], (nx, ny, nz), change_sign=False
            )
            out_V = self.boundary.from_computational_to_physical_domain(
        ...
        self.boundary.apply(
            out_U, state["x_momentum_isentropic"].values[:, :, :, 0]
        )
        self.boundary.apply(
            out_V, state["y_momentum_isentropic"].values[:, :, :, 0]
        )
    ...
    def step_integrating_sedimentation_flux(
        ...
        h_ = (
            state_now["height"]
            if state_now["height"] is not None
            else state_now["height_on_interface_levels"]
        )
        ...
        self._in_s[:nx, :ny, :] = state_now["air_isentropic_density"].values[
            :, :, :, 0
        ]
        ...
        self._out_s[:nx, :ny, nb:] = state_now[
            "air_isentropic_density"
        ].values[:, :, nb:, 0]
        self._out_qr[:, :, :nb] = state_prv[
            "mass_fraction_of_precipitation_water_in_air"
        ].values[:, :, :nb, 0]
        self._out_qr[:, :, nb:] = state_now[
            "mass_fraction_of_precipitation_water_in_air"
        ].values[:, :, nb:, 0]
    ...
    def _stencils_stepping_by_neglecting_vertical_advection_allocate_temporaries(
        self, mi, mj
    ):
        """
    ...
    def _stencil_stepping_by_coupling_physics_with_dynamics_defs(
        ...
        out_s[i, j, k] = (
            in_s_prv[i, j, k]
            - dt * (flux_s_z[i, j, k] - flux_s_z[i, j, k + 1]) / self._grid.dz
        )
        out_U[i, j, k] = (
            in_U_prv[i, j, k]
            - dt * (flux_U_z[i, j, k] - flux_U_z[i, j, k + 1]) / self._grid.dz
        )
        out_V[i, j, k] = (
            in_V_prv[i, j, k]
            - dt * (flux_V_z[i, j, k] - flux_V_z[i, j, k + 1]) / self._grid.dz
        )
        if self._moist_on:
            out_Qv[i, j, k] = (
                in_Qv_prv[i, j, k]
    ...
    def _stencil_computing_slow_tendencies_defs(
        self, dt, in_s, in_s_prv, in_qr, in_qr_prv
    ):
        """
