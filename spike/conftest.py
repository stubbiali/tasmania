...
def grid():
    domain_x = DataArray([0.0, 500.0], dims="x", attrs={"units": "km"})
    nx = 101
    domain_y = DataArray([-50.0, 50.0], dims="y", attrs={"units": "km"})
    ny = 51
    domain_z = DataArray(
        [400.0, 300.0], dims="air_potential_temperature", attrs={"units": "K"}
    )
    nz = 50
...
def isentropic_moist_sedimentation_data():
    return load_netcdf_dataset(
        "baseline_datasets/isentropic_moist_sedimentation.nc"
    )
